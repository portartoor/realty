...
<?if ($sayavka_id != ""):?>
	...
	<form id="status_change" name="STATUS_CHANGE" action="#" method="POST" enctype="multipart/form-data" onsubmit="send_status_change_form($(this));return false;">
		<div class="sayavka_content">
			...
			<input type="hidden" name="UF_REQUEST_ID" value="<?=$request_data["UF_ID".$postfix]?>">
			<input type="hidden" name="UF_AGENT" value="<?=$request_data["UF_AGENT".$postfix]?>">
			...
			<div class="field_to_fill">
				<?Helper_realty::write_select_uf_not_xml("CATEGORY",0,true,1)?>
			</div>
