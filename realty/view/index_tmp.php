...
<?
	...
	<?if($agent["UF_BITRIX_USER"]==$USER->GetID()&&!isset($_GET["interes"])):?>
		<?
			$request = HlBlockElement::GetList(13, array(), array(array("UF_REQUEST_S"=>$sayavka_id,"UF_REQUEST_F"=>$sayavka_id,"LOGIC" => "OR"),"UF_ACTIVE"=>1), Array(), 100);
	...
	<?if(isset($_GET["interes"])):?>
		...
		$dbResultObj = HlBlockElement::GetList(13, array(), array(array(array("UF_REQUEST_S"=>$request_id_s,"UF_REQUEST_F"=>$request_data["ID"]),array("UF_REQUEST_S"=>$request_data["ID"],"UF_REQUEST_F"=>$request_id_s),"LOGIC" => "OR"),"UF_ACTIVE"=>1), array(), 1);
...
<div class="webform_realty" id="request_view">
	...
	if($request_data["UF_ID"]>0&&in_array($request_data["UF_OPERATION_TYPE"],array(56,57,291))){
	?>
		<?
		if(is_dir("../../upload/1c/foto/".$request_data["UF_ID"]))
		{
			?>
			<div class="jcarousel-wrapper">
				<div class="jcarousel">
					<ul>
			<?
			$file = glob("../../upload/1c/foto/".$request_data["UF_ID"]."/*",GLOB_NOSORT);
			foreach ($file as $ky=>$vy)
			{
				$destinationFile = "../../upload/1c/slider/".$request_data["UF_ID"]."_".$ky.".jpg";
		...
		foreach($ar_help as $field_id)
		{
			...
			if(($rrr["USER_TYPE_ID"]=="enumeration"||$rrr["FIELD_NAME"]=="UF_GOAL") )
			{
				...
				if($rrr["FIELD_NAME"]=="UF_GOAL"){
					...
					Helper_realty::write_select_uf(str_replace("UF_","",$rrr["FIELD_NAME"]),1);
			}
...
?>
	...
	<?endif;?>
	<a class="big_button orange" href="tel:<?
		if($agent["UF_BITRIX_USER"]==$USER->GetID())
		...
		?>">Связаться c <?=($agent["UF_BITRIX_USER"]==$USER->GetID())?"клиентом":"агентом"?></a>
