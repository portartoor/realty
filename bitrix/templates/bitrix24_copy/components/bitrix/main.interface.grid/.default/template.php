...
if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true)
	die();
	$iblock_id_nw=intval(str_replace("WebDAV","",$arParams["GRID_ID"]));
	
?>
<script>
	BX.ready(function(){
	var ar_nw=BX.findChildren(BX(<?=$arParams["GRID_ID"]?>),{"tag":"input","class":"ckeck_list"},true);
	for(var key in ar_nw)
	{
		BX.bind(ar_nw[key], 'bxchange', function() {
			var e = this;
			BX.ajax.get("/realty/ajax/change_checklist_item.php?value="+this.id+"&checked="+this.checked,function(){
				var prnt_nw=BX.findParent(BX.findParent(e));
				var at = BX.findChildren(prnt_nw,{"tag":"input","class":"ckeck_list"},true);
				var flag=false;
				for(var key1 in at)
				{
					if(at[key1].checked==false)
					{
						flag=true;
					}
				}
				var good = BX.findChild(prnt_nw,{"tag":"a"});
				if(flag)
					BX.addClass(good,"unactive_nw")
				else
					BX.removeClass(good,"unactive_nw")
			});
		});
	}
	var ar_nw=BX.findChildren(BX(<?=$arParams["GRID_ID"]?>),{"tag":"a","class":"accept_btn"},true);
	for(var key in ar_nw)
	{
		BX.bind(ar_nw[key], 'click', function() {
			var e = this;
			event = window.event;
			BX.PreventDefault(event);
			if(BX.hasClass(e,"unactive_nw"))
				alert("Сначала необходимо заполнить чеклист!")
			else
			{
				BX.ajax.get("/realty/ajax/change_status_doc.php?value="+this.id,function(){
					console.log("567");
					var prnt_nw=BX.findParent(e);
					prnt_nw.innerHTML="<span class=\"green\">одобрено</span>";
				});
			}
		});
	}
...
<?
$arr_check=Array();
if($arParams["HEADERS"][19]["default"])
	$ttt=33;
else
	$ttt=32;
$res = CIBlockElement::GetList(Array("SORT"=>"ASC", "ID"=>"ASC"), Array("IBLOCK_ID"=>$ttt), Array("ID","NAME"));
while($ar_fields = $res->GetNext())
{
	$arr_check[$ar_fields["ID"]]=$ar_fields["NAME"];
}
?>
...
elseif($header["type"] == "checkbox")
	...
	{	
		if(strpos($header["id"],"PROPERTY_CKECKLIST")!==FALSE)
		{
			$db_props = CIBlockElement::GetProperty($iblock_id_nw, str_replace("E","",$aRow["id"]), array("sort" => "asc", "id" => "asc"),array("CODE"=>str_replace("PROPERTY_","",$header["id"])));
			$arr_t=Array();
			while($ar_props = $db_props->Fetch())
			{

				$arr_t[]=$ar_props["VALUE"];
				
			}
			foreach ($arr_check as $k=>$v)
			{
				?><label class="task-detail-checklist-item-label"><input class="ckeck_list" id="<?=$header["id"]."*".str_replace("E","",$aRow["id"])."*".$k?>" type="checkbox" <?=(in_array($k,$arr_t))?"checked":""?>><span><?=$v?></span></label><br>
				<?
			}
			?>
			<a class="accept_btn <?=(count($arr_check)==count($arr_t))?"":"unactive_nw"?> webform-small-button webform-small-button-blue task-list-toolbar-create" href="#"><span class="webform-small-button-left"></span><span class="webform-small-button-text">Одобрить</span><span class="webform-small-button-right"></span></a>
			<?
		}
		else
		{
