...
CRM = function(crmID, div, el, name, element, prefix, multiple, entityType, localize, disableMarkup)
{
	this.crmID = crmID;
	this.div = div;
	this.el = el;
...
CRM.prototype.Clear = function()
{
	...
	if (textBox)
    {
        this.div.removeChild(textBox);
    }

	var htmlBox = BX("crm-"+this.crmID+"_"+this.name+"-html-box");
	if (htmlBox)
    {
		this.div.removeChild(htmlBox);
    }
};

CRM.Set = function(el, name, subIdName, element, prefix, multiple, entityType, localize, disableMarkup)
{
	var crmID =  el.id + subIdName;
    if (obCrm[crmID])
    {
        obCrm[crmID].Clear();
        delete obCrm[crmID];
    ...
    obCrm[crmID] = new CRM(crmID, CRM.GetWrapperDivPa(el), el, name, element, prefix, multiple,  entityType, localize, disableMarkup);
    obCrm[crmID].Init();
...
CRM.prototype.Open = function ()
{
	...
	else
	{
		...
		this.popup = BX.PopupWindowManager.create("CRM-"+this.crmID+"-popup", this.el, {
...
CRM.prototype.PopupSetItem = function(id)
{
	...
	BX.ajax({
		...
		method: 'POST',
		dataType: 'json',
		data: {'MODE' : 'SEARCH', 'VALUE' : entityId, 'MULTI' : (crm.PopupPrefix? 'Y': 'N')},
...
CRM.prototype.PopupAddItem = function(arParam)
{
	...
	if (arParam['type'] == 'deal')
	{
		itemBody.appendChild(itemTitle);
		itemBody.appendChild(itemId);
		itemBody.appendChild(itemUrl);	
	}
	else
	{
		var itemDesc = document.createElement("span");
		// HACK FOR EURO HTML ENTITY
		itemDesc.innerHTML = BX.util.htmlspecialchars(arParam['desc'].replace(/&euro;/, "#EUR#")).replace("#EUR#", "&euro;");
	...
	{	
		...
		eval('BX.bind(itemBody, "click", function(event){ CRM.PopupSelectItem("'+this.crmID+'", this, arParam["type"]); BX.PreventDefault(event); })');
