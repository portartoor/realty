		...
		methods: {
			...
			addLast: function(entity)
			{
				...
				this.vars.last[entity.id] = {
					id: entity.entityId,
					type: this.getEntityType(entity.id)
				};
			},
			...
			onUnSelectDestination: function(entity)
			{
				...
				this.fireEvent('item-deselected', [{
					id: entity.entityId,
					entityType: this.getEntityType(entity.id),
					name: entity.name
				}]);
			},
			...
			initializeDialog: function()
			{
				if(this.vars.snldId == false)
				{
					...
					var parameters = {
						...
						sendAjaxSearch: modeUser || modeAll, // as we pre-load groups, ajax is required only in "user" mode
