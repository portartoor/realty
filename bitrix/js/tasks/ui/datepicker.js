...
BX.Tasks.UI.DatePicker = BX.Tasks.UI.Widget.extend({
	sys: {
		code: 'datepicker'
	},
	options: {
	...
	methods: {
		construct: function()
		{
			...
			this.vars.formatDisplay = BX.date.convertBitrixFormat(BX.message('FORMAT_DATETIME').replace(':SS', '').replace('/SS', ''));
		...
		bindEvents: function()
		{
			this.bindDelegateControl('click', 'display', this.passCtx(this.onInputClicked));
			this.bindDelegateControl('click', 'clear', BX.delegate(this.clear, this));
		},
        ...
        {
            BX[way ? 'addClass' : 'removeClass'](this.scope(), 'empty');
        }
	}
});