...
BX.Tasks.UI.UserItemSet = BX.Tasks.UI.ItemSet.extend({
	sys: {
		code: 'user-item-set'
	},
    options: {
	...
	methods: {
		construct: function()
		{
			...
			// temporal hack, after construction fixed move to bindEvents()
            ...
            this.showLoader = BX.Tasks.UI.Util.delay(this.showLoader, 500, this);
		},
		...
		bindEvents: function()
		{
			...
			BX.Tasks.UI.Util.filterFocusBlur(
				this.control('search'), 
				false,
        ...
        {
            if(this.callMethod(BX.Tasks.UI.ItemSet, 'deleteItem', arguments))
            {
		...
		extractItemValue: function(data)
		{
			...
			return id ? id : 'n'+(data.email ? BX.util.hashCode(data.email) : false); // to avoid adding duplicates when no ID defined
		},
...
BX.Tasks.UI.GroupItemSet = BX.Tasks.UI.UserItemSet.extend({
	sys: {
		code: 'group-item-set'
	},
	methods: {
...
BX.Tasks.UI.PopupItemSet = BX.Tasks.UI.ItemSet.extend({
	methods: {
		construct: function()
		{
		...
		openAddForm: function()
		{
			this.instances.window = BX.PopupWindowManager.create(this.getFullBxId('popup')+this.option('selectorCode'), this.getPopupAttachTo(), {
				autoHide : true,
				closeByEsc : true,
				content : this.control('picker-content'),
