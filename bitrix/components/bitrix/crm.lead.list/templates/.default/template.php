	...
	foreach($arResult['LEAD'] as $sKey => $arLead)
	{
		...
		$resultItem = array(
			...
			'columns' => array(
				...
				'SUM' => '<nobr>'.CCrmCurrency::MoneyToString($arLead['~OPPORTUNITY'], $arLead['~CURRENCY_ID']).'</nobr>',
				...
				'MODIFY_BY' => $arLead['~MODIFY_BY'] > 0 ?
					...
					: ''
			) + CCrmViewHelper::RenderMultiFields($arLead, "LEAD_{$arLead['ID']}_") + $arResult['LEAD_UF'][$sKey]
		);
