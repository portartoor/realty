...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if (check_bitrix_sessid())
	{
		...
		$attachedFiles = isset($_FILES['ATTACH'])? $_FILES['ATTACH']: array();
		if(!empty($attachedFiles))
			CAllFile::ConvertFilesToPost($attachedFiles, $attachedFiles);
		...
		switch ($entityTypeID) {
			case 'LEAD':
			{
				...
				{
					$arField = array('STATUS_ID' => $_POST['STATUS_ID']);
					$CCrmLead = new CCrmLead();
					$CCrmLead->Update(intval($_POST['ENTITY_ID']), $arField);
				}
			...
			case 'CONTACT':
				...
				$CCrmContact->Update(intval($_POST['ENTITY_ID']), $arField = Array());
			break;
			case 'COMPANY':
				$CCrmCompany = new CCrmCompany();
				$CCrmCompany->Update(intval($_POST['ENTITY_ID']), $arField = Array());
			break;
			case 'DEAL':
				if (isset($_POST['STAGE_ID']))
				{
					$arField = array('STAGE_ID' => $_POST['STAGE_ID']);
					$CCrmDeal = new CCrmDeal();
					$CCrmDeal->Update(intval($_POST['ENTITY_ID']), $arField);
				}
			break;
		}
	}
...
else
{
	...
	switch ($arParams['ENTITY_TYPE'])
	{
		case 'LEAD':
			...
			$arEntityAttr = $CCrmPerms->GetEntityAttr('LEAD', $arResult['ENTITY_ID']['ID']);
		...
		case 'DEAL':
			$arEntityAttr = $CCrmPerms->GetEntityAttr('LEAD', $arResult['ENTITY_ID']['ID']);
			if ($CCrmPerms->CheckEnityAccess('DEAL', 'WRITE', $arEntityAttr[$arResult['ENTITY_ID']]))
			{
				$dbRes = CCrmDeal::GetList(Array('TITLE'=>'ASC'), array('ID' => $arResult['ENTITY_ID']));
				if ($arRes = $dbRes->Fetch())
				{
					$arResult['ENTITY_TITLE'] = $arRes['TITLE'];
				...
				$arEventType = CCrmStatus::GetStatusList('DEAL_STAGE');
				foreach($arEventType as $key => $value)
				{
					if ($CCrmPerms->GetPermType('LEAD', 'WRITE', array('DEAL_STAGE'.$key)) > BX_CRM_PERM_NONE)
					{
