...
<script>
	...
	BX.message({
		...
		?>',
		MB_TASKS_TASK_EDIT_ERROR: '<?php
			echo GetMessageJS('MB_TASKS_TASK_EDIT_ERROR');
		?>',
		MB_TASKS_TASK_EDIT_BTN_REMOVE: '<?php
			echo GetMessageJS('MB_TASKS_TASK_EDIT_BTN_REMOVE');
		?>',
		MB_TASKS_TASK_EDIT_BTN_SELECT: '<?=GetMessage('MB_TASKS_TASK_EDIT_BTN_SELECT')?>'
	})
...
*/
	...
	function __MB_TASKS_TASK_EDIT_saveTask(ynBBCode)
	{
		...
		{
			app.alert({
				title  : BX.message('MB_TASKS_TASK_EDIT_ERROR'),
		...
		var taskData = {
			...
			DEADLINE:       BX('DEADLINE2SAVE').value,
...
?>
	<form method="post" onsubmit="return false<?//to prevent breaking of mobile app when submitting form?>">
		...
		<div class="task-form-main-block">
			...
			<div class="task-form-row">
				...
				<span class="task-form-row-left"><?=GetMessage('MB_TASKS_TASK_EDIT_DEADLINE')?></span><span 
						class="task-form-row-right"
					><span id="DEADLINE"
						onclick="taskEditTools.openDatePicker()"><?php
						if ($arResult['TASK']['DEADLINE'])
						{
							print($arResult['TASK']['META:FORMATTED_DATA']['DATETIME_SEXY']);
						}
						else
							echo GetMessage('MB_TASKS_TASK_EDIT_BTN_SELECT');
					?></span>
					<input type="hidden" name="DEADLINE" id="DEADLINE2SAVE" value="" />
					</span>
			</div>
			<div class="task-form-row">
				<span class="task-form-row-left"><?php
				...
				if(is_array($arResult["USER_FIELDS"]))
				{
					foreach ($arResult["USER_FIELDS"] as $arUserField)
					{
						...
						if ($diskField)
						{
							...
							'mobile',
							array(
								'PARAMS' => array("arUserField" => $arUserField),
								'EDIT' => 'Y',
	...
	<script type="text/javascript">
		...
		if(typeof taskEditTools != 'undefined')
		{
			<?
			$format = $arResult['DATETIME_FORMAT'];
			$format = array_change_key_case($format, CASE_LOWER);
			$format['internal'] = array_change_key_case($format['internal'], CASE_LOWER);
			$format['submit'] = array_change_key_case($format['submit'], CASE_LOWER);
			?>

			taskEditTools.setFormats(<?=CUtil::PhpToJsObject($format)?>);
			taskEditTools.setInitialDate(
				<?=CUtil::PhpToJsObject($arResult['TASK']['DEADLINE_PARSED'])?>
			);
		}
