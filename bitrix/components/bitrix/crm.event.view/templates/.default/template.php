...
for ($i=0, $ic=sizeof($arResult['FILTER']); $i < $ic; $i++)
{
	...
	foreach($arResult['EVENT'] as $arEvent)
	{
		...
		$arColumns = array(
			...
			'DATE_CREATE' => FormatDate('x', MakeTimeStamp($arEvent['DATE_CREATE']))
		);
	...
	else
	{
			...
			&& isset($arResult['ENTITY_ID']) && is_int($arResult['ENTITY_ID']) && $arResult['ENTITY_ID'] > 0)
		{
			$toolbarButtons[] = array(
				...
				'ONCLICK' => "javascript:(new BX.CDialog({'content_url':'/bitrix/components/bitrix/crm.event.add/box.php?FORM_ID=".strtoupper($arResult['FORM_ID'])."&ENTITY_TYPE=".$arResult['ENTITY_TYPE']."&ENTITY_ID=".$arResult['ENTITY_ID']."', 'width':'498', 'height':'275', 'resizable':false })).Show()",
				'ICON' => 'btn-new'
			);
		}
