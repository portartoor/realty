...
if (isset($arParams['ENTITY_ID']) && is_array($arParams['ENTITY_ID']))
{
	array_walk($arParams['ENTITY_ID'], create_function('&$v',  '$v = (int)$v;'));
	$arFilter['ENTITY_ID'] = $arResult['ENTITY_ID'] = $arParams['ENTITY_ID'];
}
elseif (isset($arParams['ENTITY_ID']) && intval($arParams['ENTITY_ID']) > 0)
{
	$arFilter['ENTITY_ID'] = $arResult['ENTITY_ID'] = intval($arParams['ENTITY_ID']);
}
...
$arResult['HEADERS'][] = array('id' => 'DATE_CREATE', 'name' => GetMessage('CRM_COLUMN_DATE_CREATE'), 'sort' => 'event_rel_id', 'default' => true, 'editable' => false);
...
{
	$info = CSqlHelper::GetFilterOperation($filterKey);
...
while ($arEvent = $obRes->Fetch())
{
	...
	{
		$arEvent['EVENT_DESC'] = '<div id="event_desc_short_'.$arEvent['ID'].'"><a href="#more" onclick="crm_event_desc('.$arEvent['ID'].')">'.GetMessage('CRM_EVENT_DESC_MORE').'</a></div>';
	...
	{
		$arEvent['EVENT_DESC'] = '<div id="event_desc_short_'.$arEvent['ID'].'">'.substr(($arEvent['EVENT_TEXT_1']), 0, 252).'... <a href="#more" onclick="crm_event_desc('.$arEvent['ID'].')">'.GetMessage('CRM_EVENT_DESC_MORE').'</a></div>';
	...
	{
		$arEvent['EVENT_DESC'] = '<div id="event_desc_short_'.$arEvent['ID'].'">'.substr(($arEvent['EVENT_TEXT_2']), 0, 252).'... <a href="#more" onclick="crm_event_desc('.$arEvent['ID'].')">'.GetMessage('CRM_EVENT_DESC_MORE').'</a></div>';
