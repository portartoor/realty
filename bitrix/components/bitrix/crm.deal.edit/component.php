...
if ($bEdit || $bCopy)
{
	...
	$obFields = CCrmDeal::GetList(array(), $arFilter, array());
	...
	if ($arFields === false)
	{
		...
		$arEntityAttr = $CCrmDeal->cPerms->GetEntityAttr('DEAL', array($arParams['ELEMENT_ID']));
	if ($bCopy)
	{
...
else
{
	...
	if (isset($_GET['contact_id']))
		$arFields['CONTACT_ID'] = (int) $_GET['contact_id'];
	if (isset($_GET['company_id']))
		$arFields['COMPANY_ID'] = (int) $_GET['company_id'];
}


if (($bEdit && !$CCrmDeal->cPerms->CheckEnityAccess('DEAL', 'WRITE', $arEntityAttr[$arParams['ELEMENT_ID']]) ||
	(!$bEdit && $CCrmDeal->cPerms->HavePerm('DEAL', BX_CRM_PERM_NONE, 'ADD'))))
{
	ShowError(GetMessage('CRM_PERMISSION_DENIED'));
	return;
}
...
else
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid())
	{
		...
		if(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']))
		{
			...
			{
				$arFields['CONTACT_ID'] = $_POST['CONTACT_ID'];
			}

			if(isset($_POST['COMPANY_ID']))
			{
				$arFields['COMPANY_ID'] = $_POST['COMPANY_ID'];
			}
			...
			if (empty($arResult['ERROR_MESSAGE']))
			{
				...
				if ($bEdit)
				{
					$bSuccess = $CCrmDeal->Update($arResult['ELEMENT']['ID'], $arFields);
				}
				else
				{
				...
				if($bSuccess
					...
					&& ($bEdit || !empty($arProd)))
				{
					$bSuccess = $CCrmDeal::SaveProductRows($arResult['ELEMENT']['ID'], $arProd);
					if(!$bSuccess)
					{
	...
	elseif (isset($_GET['delete']) && check_bitrix_sessid())
	{
		...
		{
			$arResult['ERROR_MESSAGE'] = '';
			if (!$CCrmDeal->cPerms->CheckEnityAccess('DEAL', 'DELETE', $arEntityAttr[$arParams['ELEMENT_ID']]))
			...
			if (empty($arResult['ERROR_MESSAGE']) && !$CCrmDeal->Delete($arResult['ELEMENT']['ID']))
...
{
	if ($CCrmDeal->cPerms->GetPermType('DEAL', $bEdit ? 'WRITE' : 'ADD', array('STAGE_ID'.$sStatusId)) > BX_CRM_PERM_NONE)
...
if (!$bEdit)
{
	...
	if ($salesListCount > 0)
	{
		...
		$strCreateOrderHtml .= '<script type="text/javascript">'.
			...
			'		}'.
			'	}'.
			'	BX.CrmEntityEditor.items["'.CUtil::JSEscape($arResult['FORM_ID']).'_CONTACT_ID"].setReadOnly(val);'.
			'	BX.CrmEntityEditor.items["'.CUtil::JSEscape($arResult['FORM_ID']).'_COMPANY_ID"].setReadOnly(val);'.
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'params' => array('sale_order_marker' => 'Y'),
	'type' => 'date_link',
	'value' => !empty($arResult['ELEMENT']['BEGINDATE']) ? ConvertTimeStamp(MakeTimeStamp($arResult['ELEMENT']['BEGINDATE']), 'SHORT', SITE_ID) : ''
);
$arResult['FIELDS']['tab_1'][] = array(
	'id' => 'CLOSEDATE',
	'name' => GetMessage('CRM_FIELD_CLOSEDATE2'),
	'type' => 'date_link',
	'value' => !empty($arResult['ELEMENT']['CLOSEDATE']) ? ConvertTimeStamp(MakeTimeStamp($arResult['ELEMENT']['CLOSEDATE']), 'SHORT', SITE_ID) : ''
);
...
{
	if ($salesListCount > 0)
		$sProductsHtml .= '<div id="ID_EXTERNAL_SALE_CREATE_BTN1" style="display:none;">'.$strCreateOrderHtmlSelect.'<input type="button" value="'.GetMessage("CRM_EXT_SALE_CD_CREATE1").'" onclick="ExtSaleGetRemoteForm('.$strCreateOrderHtmlAction.', \'CREATE\')"></div>';
}
...
if (IsModuleInstalled('bizproc'))
{
	...
	foreach ($arDocumentStates as $arDocumentState)
	{
		...
		if (strlen($arDocumentState['ID']) <= 0)
		{
			...
			ob_end_clean();
			$arResult['FIELDS']['tab_1'][] = array(
				'id' => 'BP_PARAMETERS_'.$bizProcIndex,
