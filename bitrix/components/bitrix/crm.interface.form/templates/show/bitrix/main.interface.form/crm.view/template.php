...
<?
$sidebarData = isset($arParams['~SIDEBAR_DATA']) ? $arParams['~SIDEBAR_DATA'] : array();
$sidebarTabID = isset($sidebarData['TAB_ID']) ? $sidebarData['TAB_ID'] : '';
$detailsTabID = isset($sidebarData['DETAILS_TAB_ID']) ? $sidebarData['DETAILS_TAB_ID'] : '';
$tabsExt = isset($arParams['~TABS_EXT']) ? $arParams['~TABS_EXT'] : array();

//Fix for issue #34427
if($sidebarTabID !== '' && isset($arResult['TABS_META'][$sidebarTabID]) && isset($sidebarData['TITLE']) && $sidebarData['TITLE'] !== '')
{
	$arResult['TABS_META'][$sidebarTabID]['name'] = $sidebarData['TITLE'];
}

if($detailsTabID !== '' && $arResult['TABS_META'][$detailsTabID])
{
	$arResult['TABS_META'][$detailsTabID]['name'] = GetMessage('intarface_form_detail_info');
}

$selectedTabID = $arResult["SELECTED_TAB"];
$nTabs = count($arResult["TABS"]);
$shiftSelectedTab = $selectedTabID === $sidebarTabID || $selectedTabID === $detailsTabID;

if(!$shiftSelectedTab && $selectedTabID !== ''):
	foreach($arResult["TABS"] as $tabID => $tab):
		if($tabID !== $selectedTabID)
			continue;

		$display = isset($tab['display']) ? $tab['display'] : true;
		if(!$display):
			$shiftSelectedTab = true;
			break;
		endif;
	endforeach;
endif;
...
if($shiftSelectedTab):
	...
	foreach($arResult["TABS"] as $tabID => $tab):
		$display = isset($tab['display']) ? $tab['display'] : true;
		if($display && $tabID !== $sidebarTabID && $tabID !== $detailsTabID):
			$arResult["SELECTED_TAB"] = $tabID;
			break;
		endif;
	endforeach;
endif;
if($arResult["SELECTED_TAB"] === $sidebarTabID || $arResult["SELECTED_TAB"] === $detailsTabID)
	$arResult["SELECTED_TAB"] = '';
?>
<!-- View form tabs  -->
<div id="<?=$arParams["FORM_ID"]?>_tab_block" class="bx-crm-view-tab-block"><?
	foreach($arResult["TABS"] as $tab):
		...
		<a id="<?=htmlspecialcharsbx($arParams["FORM_ID"]."_tab_". $tabID)?>" class="bx-crm-view-tab<?=$bSelected ? ' bx-crm-view-tab-active' : ''?>" href="#" onclick="bxForm_<?=$arParams["FORM_ID"]?>.SelectTab('<?=$tabID?>'); return false;" title="<?=htmlspecialcharsbx($tabTitle)?>">
...
$detailFields = array();
$sidebarID = isset($sidebarData['ID']) ? $sidebarData['ID'] : '';
$sidebarTitle = isset($sidebarData['TITLE']) ? $sidebarData['TITLE'] : '';
$sidebarIgnoredFields = isset($sidebarData['IGNORED_FIELDS']) ? $sidebarData['IGNORED_FIELDS'] : array();
foreach($arResult["TABS"] as &$tab):
	$display = isset($tab['display']) ? $tab['display'] : true;
	if(!$display)
	{
	...
	if($sidebarTabID === $tab['id']):
		$sectionData = isset($sidebarData['SECTION_DATA']) ? $sidebarData['SECTION_DATA'] : array();
		$itemData = isset($sidebarData['ITEM_DATA']) ? $sidebarData['ITEM_DATA'] : array();
		...
		foreach($tab['fields'] as &$field):
			...
			if(in_array($fieldID, $sidebarIgnoredFields, true))
				continue;

			if($field['type'] === 'section'):
				$currentSectionID = $fieldID;
				$sidebarSections[$currentSectionID] = isset($sectionData[$fieldID]) ? $sectionData[$fieldID] : array();
				$sidebarSections[$currentSectionID]['ITEMS'] = array();
				continue;
			endif;

			$value = isset($field['value']) ? $field['value'] : '';
			if($value === '' && $fieldID !== 'COMMENTS')
			{
				continue;
			}
			...
			if($currentSectionID === '')
			{
				$currentSectionID = uniqid();
				$sidebarSections[$currentSectionID] = array();
				$sidebarSections[$currentSectionID]['ITEMS'] = array();
			}

			$sectionItem = isset($itemData[$fieldID]) ? $itemData[$fieldID] : null;
			if(!is_array($sectionItem))
				$sectionItem = array();

			if(isset($sectionItem['DISPLAY']) && !$sectionItem['DISPLAY'])
			{
				continue;
			}

			if(empty($sectionItem)):
				$sectionItem['TITLE'] = array('TEXT' => $field['name']);
				$sectionItem['CONTENT'] = array('HTML' => $field['value']);
			endif;

			$sidebarSections[$currentSectionID]['ITEMS'][] = $sectionItem;
		endforeach;
		unset($field);
		...
		if($sidebarTitle === '')
		{
			$sidebarTitle = $tab['name'];
		}
		continue;
	elseif($detailsTabID  === $tab['id']):
		foreach($tab['fields'] as &$field):
			...
			if(strpos($field['id'], 'UF_') === 0
				&& preg_match('/^(\s*<[a-z0-9]+[^>]*>\s*<\/[a-z0-9]+>\s*)+$/i', $value) === 1)
			{
				continue;
			}
...
<div style="height: 100%;">
	...
	foreach($tab['fields'] as &$field):
		...
		$isEmptyField = empty($val) && $type !== 'crm_activity_list' && $field['id'] !== 'COMMENTS';
...
if(!empty($sidebarSections)):
	$viewData = isset($sidebarData['VIEW_DATA']) ? $sidebarData['VIEW_DATA'] : null;
	...
	if(!isset($viewData['SORT']))
		$viewData['SORT'] = 50;

	$this->SetViewTarget($viewData['ID'], intval($viewData['SORT']));
	$detailsContainerID = '';
	if(!empty($detailFields)):
		$detailsContainerID = "{$sidebarID}_details_container";
		CCrmViewHelper::RenderEntityInfoDetails($detailFields, $detailsContainerID);
	endif;

	CCrmViewHelper::RenderEntityInfoSidebar($sidebarID, $sidebarTitle, GetMessage('intarface_form_detail_info'), $sidebarSections, $detailsContainerID);
	$this->EndViewTarget();
endif;
?></div><?
if($arParams["SHOW_FORM_TAG"]):?>
