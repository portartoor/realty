...
else
{
	...
	$APPLICATION->IncludeComponent(
		...
		array(
			...
			"TABS" => $arTabs,
			"BUTTONS" => array(
				"custom_html" => "<input type='button' onclick='ExtSaleDoSyncStart();' id='id_do_import_btn' value='".GetMessage("BPWC_WNCTS_BTN_SYNC")."'/> ".
...
<script type="text/javascript">
	...
	function ExtSaleDoSync()
	{
		...
		var sendParams = {"id" : <?= $arResult["BP"]["ID"] ?>, "skip_bp" : "<?= ((intval($arResult["BP"]["MODIFICATION_LABEL"]) == 0) ? "Y" : "N") ?>", "skip_notify" : "<?= ((intval($arResult["BP"]["MODIFICATION_LABEL"]) == 0) ? "Y" : "N") ?>"};
		BX.ajax.get(
