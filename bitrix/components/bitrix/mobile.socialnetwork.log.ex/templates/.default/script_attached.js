	...
	if (!bAjaxCall)
	{
		BX.ready(function()
		{
			...
			)
			{
				...
				{
					BX.addCustomEvent("onUpdateSocnetCounters", oMSL.onUpdateSocnetCountersHandler);
					BX.addCustomEvent("onImUpdateCounter", oMSL.onImUpdateCounterHandler);
					...
					BX.addCustomEvent("onPull-main", BX.delegate(function(data)
					{
						...
						)
						{
							...
							BX.removeCustomEvent("onImUpdateCounter", oMSL.onImUpdateCounterHandler);
...
function __MSLOpenLogEntryNew(params, event)
{
	...
	{
		params.showMoreButton = (BX('post_more_block_' + params.log_id).style.display != 'none');
	}
...
BitrixMSL.prototype.changeCounter = function(cnt, zeroTS)
{
	if (typeof zeroTS == 'undefined')
	{
		zeroTS = 0;
	}
	...
	this.counterTimeout = setTimeout(function()
	{
		if (parseInt(cnt) > 0)
		{
			oMSL.showNotifier(cnt);
		}
		else if (
			zeroTS > 0
			&& oMSL.iframeCacheTs > 0
		)
		{
			if (zeroTS > oMSL.iframeCacheTs) // counter is null but cache is too old
			{
	...
	BMAjaxWrapper.Wrap({
		...
		'callback': function(get_data)
		{
			...
			if (typeof get_data.TEXT != 'undefined')
			{
				...
				BX('post-comments-wrap').innerHTML = ob.HTML;
...
/*					__MSLDetailPullDownInit(true);*/
				...
				oMSL.showEmptyCommentsBlockFailed(BX('post-comments-wrap'), ts, bPullDown, bMoveBottom, get_data);
			}
		},
		'callback_failure': function()
		{
...
BitrixMSL.prototype.onImUpdateCounterHandler = function(params)
{
	...
	var obCounter = params;

	oMSL.changeCounter(
		parseInt(obCounter[strCounterType]),
		(
...
BitrixMSL.prototype.onSetLFCounter = function(ob)
{
	...
	{
		oMSL.bCounterReceived = true;
		oMSL.changeCounter(parseInt(ob.value), (typeof ob.zeroCounterTs != 'undefined' ? parseInt(ob.zeroCounterTs) : null));
	}
};
