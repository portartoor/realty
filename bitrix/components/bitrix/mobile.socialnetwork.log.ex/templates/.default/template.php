...
CUtil::InitJSCore(array('date', 'ls', 'fx', 'comment_aux'));
...
else
{
	...
	AppCacheManifest::getInstance()->addAdditionalParam("UserID", $GLOBALS["USER"]->GetID());
			...
			if ($arResult["RELOAD_JSON"])
			{
				...
				AddEventHandler("main", "OnEndBufferContent", function($staticContent) use($strText, $arResult)
				{
					...
					foreach($resources['STRINGS'] as $stringContent)
					{
						...
						if (!empty($files["FILE_TIMESTAMPS"]))
						{
						...
						if (!empty($files["CSS_FILE_IMAGES"]))
						{
