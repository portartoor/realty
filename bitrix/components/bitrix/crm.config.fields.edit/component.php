...
if (!$arResult['NEW_FIELD'] && !($arResult['FIELD'] = $CCrmFields->GetFieldById($arResult['FIELD_ID'])))
{
			...
			case 'enumeration':
				...
				if(isset($_POST['LIST']) && is_array($_POST['LIST']))
				{
					foreach($_POST['LIST'] as $key => $value)
					{
						...
						if (substr($key, 0, 1) != 'n')
							$value['XML_ID'] = md5($key.$value['VALUE']);
				...
				if(isset($_POST['LIST_TEXT_VALUES']) && strlen($_POST['LIST_TEXT_VALUES']))
				{
					foreach(explode("\n", $_POST['LIST_TEXT_VALUES']) as $value_line)
					{
						...
						{
							$max_sort += 10;
							$arListMap[$value] = 'n'.$max_sort;
							$arField['LIST']['n'.$max_sort] = array(
								'SORT' => $max_sort,
								'VALUE' => $value
							);
						}
...
{
	switch ($arResult['FIELD']['USER_TYPE_ID']) {
		case 'string':
		case 'integer':
		case 'double':
