...
$arResult['HEADERS'] = array(
	...
	array('id' => 'ENTITY_TYPE_NAME', 'name' => GetMessage('CRM_COLUMN_MAIL_TEMPLATE_ENTITY_TYPE'), 'sort' => 'ENTITY_TYPE_ID', 'default' => true, 'editable' => false),
	array('id' => 'SCOPE_NAME', 'name' => GetMessage('CRM_COLUMN_MAIL_TEMPLATE_SCOPE'), 'sort' => 'SCOPE_NAME', 'default' => true, 'editable' => false),
	...
	array('id' => 'OWNER_FORMATTED_NAME', 'name' => GetMessage('CRM_COLUMN_MAIL_TEMPLATE_OWNER'), 'sort' => 'OWNER_FORMATTED_NAME', 'default' => false, 'editable' => false),
...
if(check_bitrix_sessid())
{
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_button_'.$arResult['GRID_ID']]))
	{
		...
		if($arResult['CAN_DELETE'] && $action === 'delete')
		{
			...
			else
			{
				...
				foreach($IDs as $ID)
				{
					...
					if(!is_array($fields) || intval($fields['OWNER_ID']) !== $userID)
					{
						continue;
					}
		...
		elseif($arResult['CAN_EDIT'] && $action === 'edit' && isset($_POST['FIELDS']) && is_array($_POST['FIELDS']))
		{
			...
			{
				$dbResult = CCrmMailTemplate::GetList(array(), array('=ID' => $ID), false, false, array('OWNER_ID'));
				$curFields = $dbResult->Fetch();
				if(!is_array($curFields) || (!CCrmPerms::IsAdmin() && intval($curFields['OWNER_ID']) !== $userID))
				{
					continue;
				}
				...
				if (count($fields) > 0)
				{
					if(!CCrmMailTemplate::Update($ID, $fields))
					{
						...
						if(empty($errors))
						{
							$errors[] = GetMessage('CRM_MAIL_TEMPLATE_UPDATE_GENERAL_ERROR');
						}
						ShowError(implode("\n", $errors));
						return;
					}
...
if(is_array($sort) && count($sort) > 0)
{
	...
	{
		// Sort 'string' type fields (TITLE, ENTITY_TYPE_NAME, SCOPE_NAME, OWNER_FORMATTED_NAME, IS_ACTIVE)
		usort(
			$items,
			create_function(
				'$a,$b',
