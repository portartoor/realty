...
if($bBulkMode)
{
	// this is temporal, related to UF_CRM_PORT (see below)
	$GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/js/crm/css/crm.css');
}
...
<script type="text/javascript">
	...
	BX.message({
		...
		MB_TASKS_TASK_DETAIL_BTN_ADD_FAVORITE_TASK:
			'<?=GetMessageJS('MB_TASKS_TASK_DETAIL_BTN_ADD_FAVORITE_TASK')?>',
		MB_TASKS_TASK_DETAIL_BTN_DELETE_FAVORITE_TASK:
			'<?=GetMessageJS('MB_TASKS_TASK_DETAIL_BTN_DELETE_FAVORITE_TASK')?>',
		...
		MB_TASKS_TASK_REMOVE_CONFIRM_TITLE:
			'<?php echo GetMessageJS('MB_TASKS_TASK_REMOVE_CONFIRM_TITLE'); ?>',
		MB_TASKS_TASK_DETAIL_SITE_ID:
			'<?php echo CUtil::JSEscape(htmlspecialcharsbx(SITE_DIR)); ?>',
		PATH_TO_USER_TASKS_EDIT: 
			'<?=CUtil::JSEscape(str_replace(array('#USER_ID#', '#TASK_ID#'), array($arParams['USER_ID'], 0), $arParams['PATH_TO_USER_TASKS_EDIT']))?>',
		MB_TASKS_TASK_DETAIL_TASK_ADD:
			'<?=GetMessageJS('MB_TASKS_TASK_DETAIL_TASK_ADD')?>'
	});

	ReadyDevice(
		function(){

			window.I_1_MOBILE_TASKS_VIEW_INIT_B = true;
			__MBTasks__mobile_tasks_view_init();
			window.I_2_MOBILE_TASKS_VIEW_INIT_E = true;
			...
			if ( ! $bBulkMode )
			{
				...
				MBTasks.CPT.view.resetMenu(task_id, allowed_actions);
				<?php
			}
			?>
		}
		...
		BX.addCustomEvent(
			...
			function(datum)
			{
				...
				else
				{
					// Render new data
					if (data.baseData)
						MBTasks.CPT.view.renderBaseDataStatus(data.baseData);
...
style="position:relative;">
	...
	<div class="task-card-info">
		...
		<div id="tasks-view-user-fields">

			<?if($arResult["SHOW_USER_FIELDS"]):?>
				<?foreach($arResult["USER_FIELDS"] as $field => $arUserField):?>

					<?if (empty($arUserField["VALUE"])) continue;?>

					<?$diskField = ($arUserField['USER_TYPE_ID'] == 'disk_file');?>

					<div class="task-card-info-row">
						<div class="task-card-info-left">
							<?if($diskField):?>
								<?=GetMessage('MB_TASKS_TASK_DETAIL_FILES')?>
							<?else:?>
...
?>
	<div class="task-comments-block task-comments-block-selected tasks-magic-Z" id="task-comments-block">
		<?php
