...
function __MBTasks__mobile_tasks_view_init()
{
	...
	MBTasks.CPT.view.softReset = function(id, what, toWhat)
	{
		try
		{
			var node = BX(id);
			...
			switch(what)
			{
				...
				case 'hide':
					...
					break;
				case 'className':
					node.className = typeof toWhat == 'undefined' ? '' : toWhat;
					break;
				case 'value':
					node.value = typeof toWhat == 'undefined' ? '' : toWhat;
					break;
			}
		}
		catch(e)
	...
	MBTasks.CPT.view.resetToBulk = function()
	{
		...
		var sr = MBTasks.CPT.view.softReset;

		try
		{
			sr('tasks-view-title', 'innerHTML');
			sr('tasks-view-description', 'innerHTML');
			sr('tasks-view-originator', 'innerHTML');
			sr('tasks-view-responsible', 'innerHTML');
			sr('tasks-view-originator', 'href');
			sr('tasks-view-responsible', 'href');
			sr('tasks-view-originator-work-position', 'innerHTML');
			sr('tasks-view-responsible-work-position', 'innerHTML');
			sr('tasks-view-originator-avatar', 'innerHTML');
			sr('tasks-view-responsible', 'bgImage');
			sr('tasks-view-status', 'innerHTML');
			sr('tasks-view-status-corner', 'className', 'task-view-status');
			sr('tasks-view-groupContainer', 'hide');
			sr('tasks-view-groupName', 'innerHTML');
			sr('tasks-view-deadlineBlock', 'hide');
			sr('tasks-view-deadlineValue', 'innerHTML');
			sr('tasks-view-priority', 'innerHTML');
			sr('task-card-info-notification', 'innerHTML');
			sr('task-card-info-notification', 'className', 'task-card-info-right');
			sr('tasks-view-filesList', 'innerHTML');
			sr('tasks-view-filesList', 'hide');
			sr('tasks-view-accomplicesList', 'innerHTML');
			sr('tasks-view-accomplicesBlock', 'hide');
			sr('tasks-view-auditorsList', 'innerHTML');
			sr('tasks-view-auditorsBlock', 'hide');
			sr('tasks-view-membersBlock', 'hide');

			sr('tasks-view-user-fields', 'innerHTML');

			if(BX('tasks-view-loader-area'))
				BX('tasks-view-loader-area').appendChild(BX.clone(MBTasks.CPT.view.LoadingImage));

			sr('post-comment-hidden', 'hide');
			sr('post-comment-hidden', 'innerHTML');
			...
			sr('post-comment-more', 'hide');
			sr('tasks-already-loaded-on-page', 'value');
	...
	MBTasks.CPT.view.renderBaseDataStatus = function(baseData)
	{
		if(typeof baseData != 'undefined' && typeof baseData.status_formatted_name != 'undefined')
			BX('tasks-view-status').innerHTML = BX.util.htmlspecialchars(baseData.status_formatted_name);
	};

	// Render base data in view matrix
	// this part is available at the moment, if present in the cache
	MBTasks.CPT.view.renderBaseData = function(baseData)
	{
		...
		MBTasks.CPT.view.renderBaseDataStatus(baseData);
	...
	// this part of data is available only by getting using ajax
	MBTasks.CPT.view.renderDetailData = function(data)
	{
		var filesList = BX('tasks-view-filesList');

		if ( ( ! data ) || ( ! data.task_id ) )
		{
			if(filesList)
				filesList.innerHTML = 'error';
		...
		MBTasks.CPT.view.renderUserFields(
			data.user_fields
		);

		MBTasks.CPT.view.resetMenu(data.task_id, data.actions);
	...
	MBTasks.CPT.view.resetMenu = function(taskId, allowedActions)
	{
		var menuAvailable = false;

		// object to keep run-time variables
		if(typeof MBTasks.CPT.view.vars == 'undefined')
			MBTasks.CPT.view.vars = {};

		if(typeof MBTasks.CPT.view.vars.pageMenu != 'undefined')
		{
			MBTasks.CPT.view.vars.pageMenu.hide();
		}
		...
		if (allowedActions)
		{
			...
			if (menuItems.length)
			{
				...
				MBTasks.CPT.view.vars.pageMenu = new BXMobileApp.UI.Menu({
					items: menuItems
				});

				BXMobileApp.UI.Page.TopBar.title.setCallback(function(){
					MBTasks.CPT.view.vars.pageMenu.show();
				});
	...
	MBTasks.CPT.view.renderUserFields = function(data)
	{
		var ufPlace = BX('tasks-view-user-fields');

		if(BX.type.isElementNode(ufPlace))
		{
			var chunk = '';
			if(typeof data != 'undefined')
			{
				for(var k in data)
				{
					if(BX.type.isNotEmptyString(data[k].HTML))
					{
						// very bad
						chunk +=
							'<div class="task-card-info-row user-field">'+
								'<div class="task-card-info-left">'+
									(BX.type.isNotEmptyString(data[k].TITLE) ? BX.util.htmlspecialchars(data[k].TITLE) : '&nbsp;')+
								'</div>'+
								'<div class="task-card-info-right">'+
									data[k].HTML+
								'</div>'+
							'</div>';
					}
	...
	MBTasks.CPT.view.renderFilesList = function(data)
	{
		...
		var fileList = BX('tasks-view-filesList');
		if(!fileList)
			return;
	...
	{
		if (data && data["commentsHTML"] && BX("task-comments-block"))
		{
						var ob = BX.processHTML(data["commentsHTML"], false),
				func = function(text) {
					if (BX("task-comments-block").childNodes.length > 0)
					{
						BX.ajax.processScripts(text);
					}
					else
					{
						setTimeout(function(){ func(text); }, 100);
					}
				};
			BX("task-comments-block").innerHTML = ob.HTML;
			func(ob.SCRIPT);
		}
		else if (__MB_TASKS_TASK_TOPIC_REVIEWS_ShowComments)
		{
	...
	MBTasks.CPT.view.perfomAction = function(task_id, action_system_name, datum)
	{
		...
		BX.ajax({
			...
			data: postData,
			url: MBTasks.snmRouterAjaxUrl + (
				MBTasks.snmRouterAjaxUrl.indexOf('?') >= 0 ? '&' : '?'
				) + BX.ajax.prepareData(getData),
	...
	MBTasks.CPT.view.createMenu = function(task_id, allowed_actions)
	{
		var menu_items = [
			{
				...
				{
					app.showModalDialog({
						url: BX.message('PATH_TO_USER_TASKS_EDIT')
					});
				}, this)
			}
		];
		...
		allowed_actions.forEach(function(action, index, array){
			...
			if (action.public_name == 'add_favorite' || action.public_name == 'delete_favorite') {
				menu_items.push({
					name: BX.message('MB_TASKS_TASK_DETAIL_BTN_'+action.system_name.toString().toUpperCase()+'_TASK'),
