...
class MBTasksCptDetailAjax
{
	...
	)
	{
		...
		$APPLICATION->IncludeComponent("bitrix:forum.comments", "", array(
				"FORUM_ID" => $arComponentParams['FORUM_ID'],
				"ENTITY_TYPE" => "TK",
				"ENTITY_ID" => $arComponentParams['TASK_ID'],
				"ENTITY_XML_ID" => "TASK_".$arComponentParams['TASK_ID'],
				"URL_TEMPLATES_PROFILE_VIEW" => $arComponentParams['URL_TEMPLATES_PROFILE_VIEW'],
				"CACHE_TYPE" => "Y",
				"CACHE_TIME" => 3600,
				"IMAGE_HTML_SIZE" => 400,
				"DATE_TIME_FORMAT" => $arComponentParams["DATE_TIME_FORMAT"],
				"SHOW_RATING" => $arComponentParams['SHOW_RATING'],
				"RATING_TYPE" => $arComponentParams['RATING_TYPE'],
				'PREORDER' => 'N',
				"PERMISSION" => "M",
				"NAME_TEMPLATE" => $arComponentParams["NAME_TEMPLATE"]
			),
			null,
			array('HIDE_ICONS' => 'Y')
		);
		...
		return (
			array(
				'task_id'       => (int) $taskId,
				'commentsHTML' => \Bitrix\Main\Page\Asset::getInstance()->getJs().$html,
	...
	public static function ToggleFavorite($taskId, $way)
	{
		if ( ! $GLOBALS['USER']->IsAuthorized() || !($taskId = intval($taskId)))
			return (false);
		...
		if (isset($tasksData['TASK_ID']) && check_bitrix_sessid())
		{
			...
			if ( ( ! $bCreateMode ) && $bDelegate )
			{
				...
				if (intval($delegateToUser) !== $curUserId)
				{
					...
					// this will throw an exception in case of bad permissions
					$task = new CTaskItem($tasksData['TASK_ID'], $curUserId);
	...
	public static function PrepareDetailData($task_id, $arRes, $parameters)
	{
		global $APPLICATION;
		...
		// fetch user fields (in read-only mode currently)
		if(is_array($parameters['USER_FIELDS']))
		{
			foreach($parameters['USER_FIELDS'] as $field => $desc)
			{
				if(empty($arRes['TASK'][$field]))
					continue;
				...
				$APPLICATION->IncludeComponent(
					...
					$desc["USER_TYPE"]["USER_TYPE_ID"],
					array("arUserField" => $desc, 'MOBILE' => 'Y'),
					null,
					array("HIDE_ICONS"=>"Y")
				);
				...
				{
					// desktop version of CRM UF is slightly not sutable for using in mobile, but we can do some changes here until the MOBILE VERSION is ready

					// replace urls
					$html = preg_replace(
						'#/crm/(deal|lead|company|contact){1}/show/(\d+)/#',
						'/mobile/crm/$1/view.php?$1_id=$2',
						$html
					);
				...
				elseif($field == 'UF_TASK_WEBDAV_FILES')
				{
					// for disk files we add absent title
					if((string) $desc['LIST_COLUMN_LABEL'] == '')
					{
						$desc['LIST_COLUMN_LABEL'] = GetMessage('MB_UF_TASK_WEBDAV_FILES_TITLE');
					}
				}

				$arTaskData['detailsData']['user_fields'][$field] = array(
					'TITLE' => $desc['LIST_COLUMN_LABEL'],
					'HTML' => trim($html)
				);
			}
		}
	...
	public static function ChangeTaskStatus($action_name, $curUserId, $arTask, $task_id, $arParams)
	{
		...
		return array(
			...
			'result'        => array(
				...
				'data' => static::getTaskData($task_id, $arParams)
			)
		);
	}

	public static function getTaskData($taskId, $arParams)
	{
		...
		// user fields...
		$parameters["USER_FIELDS"] = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("TASKS_TASK", $taskId, LANGUAGE_ID);

		$arTaskData = array(
			'task_id' => $taskId
		);

		$arTaskData['detailsData'] = MBTasksCptDetailAjax::PrepareDetailData($taskId, $arRes, $parameters);
...
isset($_REQUEST['user_id'])
	...
	&& (
		...
		|| (
			...
			&& isset($_REQUEST['action_name'])
			&& (
				(
					($_REQUEST['action_name'] === 'edit')
					&& isset($_REQUEST['taskData'])
				)
				||
				(
					($_REQUEST['action_name'] === 'delegate')
					&& isset($_REQUEST['taskData'])
				)
				||
				(
					($_REQUEST['action_name'] === 'add_comment')
					&& isset($_REQUEST['NEW_COMMENT_TEXT'])
				)
			)
		)
	)
...
//s oundex($_REQUEST['subject']);
	$userId = (int) $_REQUEST['user_id'];
	$taskId = $task_id = (int) $_REQUEST['task_id'];
	$action_name = 'view';
	$newComment = false;
	if (
		($action === 'perfom_action')
	...
	)
	{
		...
		$rc = MBTasksCptDetailAjax::SaveTask($_REQUEST['taskData']);
	...
	)
	{
		...
		$newComment = (string) $_REQUEST['NEW_COMMENT_TEXT'];
	}
	...
	if ($taskId > 0)
	{
		...
		if ($_REQUEST['subject'] === 'BASE')
			$bLoadBaseData = true;

		if ($_REQUEST['subject'] === 'BASE_AND_DETAIL_AND_COMMENTS')
		{
			$bLoadBaseData     = true;
			$bLoadDetailData   = true;
			$bLoadCommentsData = true;
		}
		elseif ($_REQUEST['subject'] === 'DETAIL_AND_COMMENTS')
		{
			$bLoadDetailData   = true;
			$bLoadCommentsData = true;
		}
		elseif ($_REQUEST['subject'] === 'DETAIL')
			$bLoadDetailData = true;
		elseif ($_REQUEST['subject'] === 'COMMENTS')
			$bLoadCommentsData = true;
		...
		$arFilter = false;
		if (isset($_REQUEST['CHECK_FILTER_ID']))
		{
			$filterPresetId = (int) $_REQUEST['CHECK_FILTER_ID'];
			$oFilter = CTaskFilterCtrl::GetInstance($userId);
			$arFilter = $oFilter->GetFilterPresetConditionById($filterPresetId);
		}
		elseif((string) $_REQUEST['CHECK_FILTER'] != '')
		{
			try
			{
				$signer = new Signer();
				$arFilter = unserialize(base64_decode($signer->unsign($_REQUEST['CHECK_FILTER'])));
			}
		...
		// user fields...
		$parameters["USER_FIELDS"] = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("TASKS_TASK", $task_id, LANGUAGE_ID);
		...
		if ($bLoadDetailData)
			$arTaskData['detailsData'] = MBTasksCptDetailAjax::PrepareDetailData($task_id, $arRes, $parameters);

		if ($bLoadCommentsData
			&& isset($_REQUEST['PATH_TO_FORUM_SMILE'])
			&& isset($_REQUEST['PATH_TEMPLATE_TO_USER_PROFILE'])
//			&& isset($_REQUEST['AVA_WIDTH'])
//			&& isset($_REQUEST['AVA_HEIGHT'])
		)
		{
			$arAlreadyLoadedComments = false;
			// Unset messages, that are already loaded in interface
			if (isset($_REQUEST['comments_already_loaded'])
				&& (strlen($_REQUEST['comments_already_loaded']) > 0)
			)
			{
				$arAlreadyLoadedComments = explode('|', $_REQUEST['comments_already_loaded']);
			}
		
			$arAvatarSize = array(
				'width'  => (int) $_REQUEST['AVA_WIDTH'], 
				'height' => (int) $_REQUEST['AVA_HEIGHT']
			);

			if (
				isset($_REQUEST['DEFAULT_MESSAGES_COUNT'])
				&& ($_REQUEST['DEFAULT_MESSAGES_COUNT'] > 0)
			)
			{
				$defMessagesCount = (int) $_REQUEST['DEFAULT_MESSAGES_COUNT'];
			}
			else
				$defMessagesCount = 3;
			...
			$arTaskData['commentsData'] = MBTasksCptDetailAjax::PrepareCommentsData(
				$arAvatarSize, 
				$_REQUEST['PATH_TO_FORUM_SMILE'], 
				$arRes['FORUM_ID'],
				$_REQUEST['task_id'],
				$_REQUEST['user_id'],
				$_REQUEST['PATH_TEMPLATE_TO_USER_PROFILE'],
				$arRes['NAME_TEMPLATE'],
				$lastViewedDate,
				$_REQUEST['DATE_TIME_FORMAT'],
				$arAlreadyLoadedComments,
				$newComment,
				$defMessagesCount
			);
...
)
{
	...
	if ($bActionAllowed)
	{
		$taskIdPost = (int) $_POST['task_id'];

		if ($_POST['action_name'] === 'remove')
		{
			$arResult = MBTasksCptDetailAjax::RemoveTask($taskIdPost);
		}
		elseif($_POST['action_name'] === 'add_favorite' || $_POST['action_name'] === 'delete_favorite')
		{
			MBTasksCptDetailAjax::ToggleFavorite($taskIdPost, $_POST['action_name'] === 'add_favorite');

			$arResult = array(
				'bResultInJson' => false,
				'result'        => array(
					'rc'   => 'executed',
					'action_perfomed' => (string) $_POST['action_name'],
					'data' => MBTasksCptDetailAjax::getTaskData($taskIdPost, $arParams)
				)
			);
		}
		else
...
{
	echo CUtil::PhpToJsObject($arResult['result']);
}
