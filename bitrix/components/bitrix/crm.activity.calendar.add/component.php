...
switch ($arResult['ENTITY_TYPE'])
{
	...
	default:
	case 'LEAD':
		$obRes = CCrmLead::GetList(array('ID' => 'ASC'), $arFilter, array('ID', 'ASSIGNED_BY_ID')); $arResult['ENTITY_TYPE'] = 'LEAD';
		break;
}

$arID = array();
while ($arRow = $obRes->Fetch())
	$arID[$arRow['ID']] = isset($arRow['CREATED_BY_ID']) ? $arRow['CREATED_BY_ID'] : $arRow['ASSIGNED_BY_ID'];
