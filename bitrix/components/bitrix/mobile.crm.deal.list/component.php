...
$arResult['STAGE_LIST'] = CCrmViewHelper::GetDealStageInfos();//CCrmStatus::GetStatusList('DEAL_STAGE');
$allStageList = array();
$i=0;
foreach($arResult['STAGE_LIST'] as $id => $info)
{
	if (!isset($info["COLOR"]))
	{
		...
		$arResult['STAGE_LIST'][$id]['COLOR'] = $info["COLOR"];
	}

	$allStageList["s".$i] = array(
...
while($item = $dbRes->GetNext())
{
	...
	if ($isEditPermitted)
	{
		$arActions[] = array(
			...
			"ONCLICK" => "BX.Mobile.Crm.List.showStatusList(" . $item['ID'] . ", " . CUtil::PhpToJSObject($allStageList) . ", 'onCrmDealDetailUpdate')",
		);
