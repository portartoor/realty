...
/*		$arActions[] =  array(
		...
		if ($arResult['ACTIVITY_ENTITY_LINK'] == 'Y')
		{
			...
			$arColumns['ENTITY_TITLE'] = !empty($arCal['ENTITY_TITLE'])?
				...
				'<script type="text/javascript">BX.tooltip('.$arCal['ENTITY_ID'].', "balloon_'.$arResult['GRID_ID'].'_I_'.$arCal['REL_ID'].'", "/bitrix/components/bitrix/crm.'.strtolower($arCal['ENTITY_TYPE']).'.show/card.ajax.php", "crm_balloon'.($arCal['ENTITY_TYPE'] == 'LEAD' || $arCal['ENTITY_TYPE'] == 'DEAL'? '_no_photo': '_'.strtolower($arCal['ENTITY_TYPE'])).'", true);</script>'
				: '';
		}
		else
		{
