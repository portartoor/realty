	...
	&& check_bitrix_sessid())
{
	...
	else
	{
		...
		elseif(isset($_POST['save']) || isset($_POST['apply']))
		{
			...
			$arMailBoxData['USE_TLS'] = $arSettings['USE_TLS'] = $_POST['SSL'] == 'Y' ? 'Y' : 'N';
			$arMailBoxData['PORT'] = $arSettings['PORT'] = isset($_POST['PORT']) ? trim($_POST['PORT']) : ($arMailBoxData['USE_TLS'] === 'Y' ? 995 : 110);
...
else
{
	...
	{
		$arSettings['PORT'] = isset($arCurrentMailBox['PORT']) ? $arCurrentMailBox['PORT'] : ($arSettings['USE_TLS'] === 'Y' ? 995 : 110);
	}
...
while($arMailBox = $rsMailBoxes->Fetch())
{
	...
	if(isset($arSettings['MAILBOX_ID']) &&  $arSettings['MAILBOX_ID'] === $mailBoxID)
	{
		...
		if(isset($arSettings['USE_TLS']))
		{
			$arMailBoxSettings['SSL'] = $arSettings['USE_TLS'];
		}
		if(isset($arSettings['PERIOD_CHECK']))
		{
