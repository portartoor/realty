...
<form name="configPostForm" id="configPostForm" method="POST" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
	...
	<table id="content-edit-form-config" class="content-edit-form" cellspacing="0" cellpadding="0">
		...
		<tr>
			...
			<?if ($arResult["LICENSE_TYPE"] == "team" || $arResult["LICENSE_TYPE"] == "company" || $arResult["LICENSE_TYPE"] == "edu"):
			...
			<td class="content-edit-form-field-input">
				...
				<div id="RIGHTS_div">
					...
					foreach($arToAllRights as $right)
					{
						...
						?><div data-bx-right="<?=$right?>" class="toall-right"><span><?=(!empty($arNames[$right]["provider"]) ? $arNames[$right]["provider"].": " : "").$arNames[$right]["name"]?>&nbsp;</span><a href="javascript:void(0);" onclick="B24ConfigsLiveFeedObj.DeleteToAllAccessRow(this);" class="access-delete" title="<?=GetMessage("CONFIG_TOALL_DEL")?>"></a></div><?
					}
					?>
				</div>
			...
			<td class="content-edit-form-field-input">
				...
				<div id="chat_RIGHTS_div">
					...
					foreach($arChatToAllRights as $right)
					{
						...
						?><div data-bx-right="<?=$right?>" class="toall-right"><span><?=(!empty($arNames[$right]["provider"]) ? $arNames[$right]["provider"].": " : "").$arNames[$right]["name"]?>&nbsp;</span><a href="javascript:void(0);" onclick="B24ImChatObj.DeleteToAllAccessRow(this);" class="access-delete" title="<?=GetMessage("CONFIG_TOALL_DEL")?>"></a></div><?
					}
					?>
				</div>
...
<?
elseif (!in_array($arResult["LICENSE_TYPE"], array("team", "company", "demo", "edu", "tf"))):
	$disabled = true;
							...
							if (lock3)
							{
								BX.bind(lock3, "click", function(){
									B24.licenseInfoPopup.show('network-available', '<?=GetMessageJS("CONFIG_NETWORK_AVAILABLE_TITLE")?>',
										'<?=GetMessageJS("CONFIG_NETWORK_AVAILABLE_TEXT", array("#PRICE#" => $arResult["PROJECT_PRICE"]))?>');
								});
							}
						});
					</script>
...
?>
		<tr>
			<td class="content-edit-form-header " colspan="3">
				<div class="content-edit-form-header-wrap content-edit-form-header-wrap"><?=GetMessage('CONFIG_NAME_CHANGE_SECTION')?></div>
			</td>
		</tr>
		<tr>
...
<!-- logo -->
<?
if ($arResult["IS_BITRIX24"] && in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "edu")))
{
...
<table id="content-edit-form-config" class="content-edit-form" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td class="content-edit-form-field-input" colspan="2">
			...
			<form name="configLogoPostForm" id="configLogoPostForm" method="POST" action="/settings/configs/" enctype="multipart/form-data">
				<?=bitrix_sessid_post();?>
				<div id="config-wait" style="display:none;"><img src="<?=$this->GetFolder();?>/images/wait.gif"/></div>
				<div class="config-webform-field-upload" style="display: inline-block; !important; margin-top:10px;vertical-align: middle;" onmouseup="BX.removeClass(this,'content-edit-form-button-press')" onmousedown="BX.addClass(this, 'content-edit-form-button-press')">
...
<table class="content-edit-form" cellspacing="0" cellpadding="0">
	<tr>
		<td class="content-edit-form-buttons" style="border-top: 1px #eaeae1 solid; text-align:center">
			<span onclick="BX.addClass(this, 'webform-button-wait webform-button-active'); BX.submit(BX('configPostForm')); this.onclick = '';" class="webform-button webform-button-create">
