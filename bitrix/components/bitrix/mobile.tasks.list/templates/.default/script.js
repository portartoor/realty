function __MBTasksCPTListInit(opts)
{
	MBTasks.CPT.List.vars = {};
					...
					function(){
						try
						{
							BXMobileApp.PageManager.loadPageUnique({
								url: MBTasks.CPT.List.snmrouterPath,
					...
					function(){
						try
						{
							BXMobileApp.PageManager.loadPageBlank({
								url: MBTasks.CPT.List.taskViewHrefTemplate.replace('#TASK_ID#', taskId),
								bx24ModernStyle: true
							});
						}
						catch(e)
	...
	MBTasks.CPT.List.loadBaseTaskData = function(taskId, callback)
	{
		...
		var postData = {
			...
			CHECK_FILTER: MBTasks.CPT.List.FILTER,
	...
	MBTasks.CPT.List.onTasksFilterChangeHandler = function(data)
	{
		...
		MBTasks.CPT.List.reload(data);
	};

	MBTasks.CPT.List.reload = function(data)
	{
		// clean up old values, whatever they were
		// replace with BX.util.remove_url_param() later
		var url = MBTasks.CPT.List.remove_url_param(document.location.href, [
			'SWITCH_TO_FILTER_PRESET_ID', // old one, should be sure it removed
			'F_FILTER_SWITCH_PRESET',
			'F_STATE\\[\\d*\\]',
			'F_STATE%5B\\d*%5D',
			'F_CANCEL',
			'GROUP_ID'
		...
		if(typeof data.filter_preset_id != 'undefined') // backward compatibility
		{
			params['F_FILTER_SWITCH_PRESET'] = parseInt(data.filter_preset_id);
		}
		if(typeof data.filter != 'undefined' && BX.type.isPlainObject(data.filter))
		{
			if(typeof data.filter['F_FILTER_SWITCH_PRESET'] != 'undefined')
				params['F_FILTER_SWITCH_PRESET'] = parseInt(data.filter['F_FILTER_SWITCH_PRESET']);

			if(typeof data.filter.F_STATE != 'undefined' && BX.type.isArray(data.filter.F_STATE))
			{
				for(var i = 0; i < data.filter.F_STATE.length; i++)
				{
					params['F_STATE['+i+']'] = data.filter.F_STATE[i];
				}
			}

			if(typeof data.filter.GROUP_ID != 'undefined')
				params['GROUP_ID'] = parseInt(data.filter.GROUP_ID);
		}

		params['F_CANCEL'] = 'Y';

		app.reload({url: BX.util.add_url_param(url, params)});
	};

	// temporal fix from "main" to aviod module dependency
	MBTasks.CPT.List.remove_url_param = function(url, param)
	{
		if (BX.type.isArray(param))
		{
			for (var i=0; i<param.length; i++)
			{
				url = MBTasks.CPT.List.remove_url_param(url, param[i]);
			}
		}
		else
		{
			...
			if((pos = url.indexOf('?')) >= 0 && pos != url.length-1)
			{
				...
				params = params.replace(new RegExp('(^|&)'+param+'=[^&#]*', 'ig'), '');
	...
	MBTasks.CPT.List.MenuCreate = function(opts)
	{
		additionalItems = opts.menu.additionalItems;
		...
		if(BX.type.isArray(additionalItems))
		{
			for(var i = 0; i < additionalItems.length; i++)
			{
				var act = additionalItems[i].action;

				(function(additionalItems, act){

					additionalItems[i].action = function(){
						MBTasks.CPT.List[act.f].apply(this, act.args);
					};

				})(additionalItems, act);

				var code = additionalItems[i].code;
				delete(additionalItems[i].code);

				if(code == 'all' || code == 'in_progress' || code == 'deferred' || code == 'completed')
					additionalItems[i].image = opts.templateFolder+'/img/category-'+code+'.png';

				menuItems.push(additionalItems[i]);
			}
		}
		...
		menuItems.push({
			...
			icon: 'settings',
			action: function() {
				BXMobileApp.PageManager.loadPageUnique({url: BX.message('PATH_TO_TASKS_FILTER'), bx24ModernStyle: true, data: {
					currentFilter: BX.message('CURRENT_FILTER')
				}});
			}
		});

		// Prepare menu items accord to allowed actions
		MBTasks.CPT.List.vars.pageMenu = new BXMobileApp.UI.Menu({
			items: menuItems
		});

		BXMobileApp.UI.Page.TopBar.title.setCallback(function(){
			MBTasks.CPT.List.vars.pageMenu.show();
		});
	}
	...
	*/
	...
	MBTasks.CPT.List.MenuCreate(opts);
	...
	{
		BXMobileApp.UI.Page.TopBar.title.setText(BX.message('PAGE_TITLE'));
