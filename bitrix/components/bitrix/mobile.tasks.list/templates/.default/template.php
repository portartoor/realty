...
if ( ! class_exists('CTasksMobileTasksListNS') )
{
	class CTasksMobileTasksListNS extends CTasksMobileTasksListNsAbstract
	{
				...
				return((function(){
					try
					{
						BXMobileApp.PageManager.loadPageUnique({
							url: '<?=$snmrouterPath?>',
							bx24ModernStyle: true,
							data: MBTasks.CPT.List.tasksBaseDataCache['t' + <?=((int) $arTask['ID'])?>]
						});
					}
					catch(e)
					{
		...
		public static function renderGroupItem ($bShow, $arGroup, $pathTemplate, $userId, $arResult)
		{
			$path = str_replace(
			...
			// this is required to let ios know, that pages for different roles and categories are REALLY different pages
			// this will be deprecated when mobile.tasks.list will move to single-page way
			if(is_array($arResult['FILTER_QUERY']))
			{
				$query = $arResult['FILTER_QUERY'];
				if(is_array($query['F_STATE']))
				{
					$s = $query['F_STATE'];
					unset($query['F_STATE']);
			...
			$path = CHTTP::urlAddParams($path, $query);
...
if(isset($arResult['FILTER_QUERY']['F_STATE'][0]))
	$currentFilter = $arResult['FILTER_QUERY']['F_STATE'][0];
else if(isset($arResult['FILTER_QUERY']['F_FILTER_SWITCH_PRESET']))
	$currentFilter = $arResult['FILTER_QUERY']['F_FILTER_SWITCH_PRESET'];
else
	$currentFilter = '';
?>
<script>
...
BX.message({
	PAGE_TITLE: 
		'<?php echo GetMessageJS('MB_TASKS_GENERAL_TITLE'); ?>',
	...
	CURRENT_FILTER: '<?=CUtil::JSEscape($currentFilter)?>'
});
...
<?
	$catsToShow = array('ALL', 'IN_PROGRESS', 'DEFERRED', 'COMPLETED');
	$additionalItems = array();

	if($arResult['VIEW_STATE']['SECTION_SELECTED']['ID'] == CTaskListState::VIEW_SECTION_ROLES && is_array($arResult['VIEW_STATE']['TASK_CATEGORIES']))
	{
		$commonFilter = array();
		if(isset($arResult['FILTER_QUERY']['GROUP_ID']))
			$commonFilter['GROUP_ID'] = intval($arResult['FILTER_QUERY']['GROUP_ID']);

		$commonFilter['GROUP_LIST_MODE'] = $_REQUEST['GROUP_LIST_MODE'] == 'Y' ? 'Y' : 'N';

		foreach($catsToShow as $category)
		{
			if(isset($arResult['VIEW_STATE']['TASK_CATEGORIES']['VIEW_TASK_CATEGORY_'.$category]))
			{
				$cat = $arResult['VIEW_STATE']['TASK_CATEGORIES']['VIEW_TASK_CATEGORY_'.$category];

				$additionalItems[] = array(
					'name' => ToUpper(substr($cat['TITLE'], 0, 1)).substr($cat['TITLE'], 1),
					'code' => ToLower($category),
					'arrowFlag' => false,
					'action' => array(
						'f' => 'reload',
						'args' => array(
							array(
								'filter' => array_merge($commonFilter, array(
									'F_STATE' => array(
										'sR'.base_convert(intval($arResult['VIEW_STATE']['ROLE_SELECTED']['ID']), 10, 32),
										'sC'.base_convert(intval($cat['ID']), 10, 32)
									)
								))
							)
						)
...
(function(){
	...
	MBTasks.CPT.List = {
		...
		FILTER: '<?=CUtil::JSEscape($arResult['FILTER_ENCODED_SIGNED'])?>',
		snmrouterPath: '<?php
			echo CUtil::JSEscape(
				str_replace(
					array('#USER_ID#', '#user_id#'),
...
<div id="asdfsdfgdsf" class="task-title">
	...
	// Button that allows to switch between flat and group modes
	// don't show the button, when we are in group list mode and some group selected
	if (
		! (
