		...
		foreach ($arResult['MESSAGES'] as $arComment)
		{
			...
			?><div id="tasks-comment-block-<?php echo $arComment['ID']; ?>" class="post-comment-block<?=($bUnread ? " post-comment-new" : "")?>"><?
				?><div class="post-user-wrap"><?
					?><div class="avatar" <?=(strlen($arComment['AUTHOR_PHOTO']) > 0 ? " style=\"background:url('".$arComment['AUTHOR_PHOTO']."') no-repeat;background-size: cover;\"" : "")?>></div><?
...
?>
	<script>
		...
		BXMobileApp.UI.Page.TextPanel.show({
			placeholder: '<?=GetMessageJS('MB_TASKS_TASK_FORUM_TOPIC_REVIEWS_EDIT_COMMENT_PLACEHOLDER')?>',
			button_name: '<?=GetMessageJS('MB_TASKS_TASK_FORUM_TOPIC_REVIEWS_SEND_COMMENT_BUTTON')?>',
			action:function(message)
			{
				if(typeof message != 'undefined' && BX.type.isNotEmptyString(message))
					tasksNativeInputCallback(message);
			}
		});
	</script>
