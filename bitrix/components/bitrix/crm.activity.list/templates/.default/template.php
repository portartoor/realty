		...
		<tbody>
			...
			for($i = 0; $i < $count; $i++)
			{
				...
				$editorItem = array(
					...
					'end' => isset($item['~START_TIME']) ? ConvertTimeStamp(MakeTimeStamp($item['~END_TIME']), 'FULL', SITE_ID) : '',
			...
			<tr id="<?= htmlspecialcharsbx($rowID) ?>" class="<?=htmlspecialcharsbx($rowClass)?>" style="<?= $showTop > 0 && $processed > $showTop ? 'display:none;' : '' ?>">
				...
				<td> <!--End time-->
					<? $end = isset($item['~END_TIME']) ? MakeTimeStamp($item['~END_TIME']) : null; ?>
					<span <?= $item['~COMPLETED'] !== 'Y' && $end !== null && $end < $now ? 'style="color:#ff0000;"' : '' ?>>
						<?= $end !== null ? htmlspecialcharsbx(CCrmComponentHelper::TrimDateTimeString(FormatDate('FULL', $end))) : '' ?>
					</span>
				</td>
				<td> <!--Responsible-->
			<span>
