...
foreach($arResult['ITEMS'] as &$item)
{
	...
	if($isEditable)
	{
		...
		if($item['CAN_EDIT'] && $itemTypeID !== CCrmActivityType::Email) //Email is always COMPLETED
		{
	...
	if($completed === 'Y'):
		...
		$completedTitle = GetMessage($item['CAN_EDIT'] ? 'CRM_ACTION_CLICK_TO_COMPLETE' : 'CRM_ACTION_NOT_COMPLETED');
		$completedOnClick = $item['CAN_EDIT'] ? 'BX.CrmActivityEditor.items[\''.$gridEditorID.'\'].setActivityCompleted('.$item['ID'].', true); return false;' : 'return false;';
	endif;
	...
	if($arResult['DISPLAY_CLIENT'] && (empty($arSelect) || in_array('CLIENT', $arSelect, true)))
	{
		...
		{
			foreach($item['COMMUNICATIONS'] as &$comm)
			{
				...
				$entityID = $commOwnerID;
				$entityTypeID = $commOwnerTypeID;
				...
				if($entityTypeID === CCrmOwnerType::Company)
				{
					...
					{
						$entityTitle = $settings['COMPANY_TITLE'];
					}
				...
				if($entityTypeID === CCrmOwnerType::Contact)
				{
					...
					$entityTitle = CUser::FormatName(CSite::GetNameFormat(false),
						array(
							'LOGIN' => '',
							'NAME' => $settings['NAME'],
		...
		elseif($ownerTypeID === CCrmOwnerType::Company || $ownerTypeID === CCrmOwnerType::Contact)
		{
			$columnHtml= CCrmViewHelper::PrepareEntityBaloonHtml(
				array(
					...
					'PREFIX' => "{$arResult['UID']}_{$item['~ID']}_CLIENT",
					'TITLE' => CCrmOwnerType::GetCaption($ownerTypeID, $ownerID),
					'SHOW_URL' => CCrmOwnerType::GetShowUrl($ownerTypeID, $ownerID)
				)
			);
		}
...
if($enableToolbar)
{
	...
	if($isEditable && $arResult['OWNER_TYPE'] !== '' && $arResult['OWNER_ID'] !== '')
	{
		$toolbarButtons[] = array(
			...
			'ONCLICK' => "javascript:(new BX.CDialog({'content_url':'/bitrix/components/bitrix/crm.event.add/box.php?FORM_ID=".strtoupper($arResult['FORM_ID'])."&ENTITY_TYPE=".$arResult['OWNER_TYPE']."&ENTITY_ID=".$arResult['OWNER_ID']."', 'width':'498', 'height':'275', 'resizable':false })).Show()",
			'ICON' => 'btn-new'
		);
	}
...
$APPLICATION->IncludeComponent(
	...
	array(
		...
		'AJAX_INIT_EVENT' => $arResult['AJAX_INIT_EVENT'],
