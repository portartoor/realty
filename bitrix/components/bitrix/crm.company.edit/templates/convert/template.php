...
<tr  style="background-color: #ffe27f;">
	<td class="bx-padding" colspan="2" style="background:none; text-align: center; font-weight: bold;">
			<input type="radio" type="checkbox" value="Y" id="CONVERT_COMPANY_Y" name="CONVERT_COMPANY" <?=($bChecked ? 'checked' : '')?> 
			onclick="BX('COMPANY_form').style.display = (BX('COMPANY_form').style.display == 'none' ? 'block' : 'none');
		BX('COMPANY_choise').style.display = (BX('COMPANY_choise').style.display == 'none' ? '' : 'none');" /><label for="CONVERT_COMPANY_Y" style="padding-right: 52px;"><?=GetMessage('CRM_FIELD_CONVERT_COMPANY')?></label>
			<input type="radio" type="checkbox" value="N" id="CONVERT_COMPANY_N" name="CONVERT_COMPANY" <?=(!$bChecked ? 'checked' : '')?>
			onclick="BX('COMPANY_form').style.display = (BX('COMPANY_form').style.display == 'none' ? 'block' : 'none');
		BX('COMPANY_choise').style.display = (BX('COMPANY_choise').style.display == 'none' ? '' : 'none');"/><label for="CONVERT_COMPANY_N"><?=GetMessage('CRM_FIELD_CONVERT_COMPANY_LIST')?></label>
	</td>
</tr>
<tr id="COMPANY_choise" style="background-color: #FFF6D8;display:<?=(!$bChecked ? '' : 'none')?>">
	<td class="bx-field-name bx-padding" style="background:none"><span class="required">*</span><?=GetMessage('CRM_FIELD_CONVERT_COMPANY_CHOISE')?>:</td>
