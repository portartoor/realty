...
if (SITE_TEMPLATE_ID == 'bitrix24')
{
	...
	<div class="pagetitle-container pagetitle-flexible-space">
		...
		<span class="intranet-toolbar-button" onclick="mb.modeMenu(this); ">
			<span class="intranet-toolbar-button-text">
				<?=getMessage('user' == $arResult['MODE'] ? 'INTR_MAIL_MANAGE_MODE_USER' : 'INTR_MAIL_MANAGE_MODE_MAILBOX') ?>
			</span>
			<span class="intranet-toolbar-button-icon intranet-toolbar-button-icon-arrow-down"></span>
		</span>
	</div>

	<span class="pagetitle-container pagetitle-align-right-container">
		<? if ($hasOptions1 || $hasOptions2): ?>
			<span class="intranet-toolbar-button manage-page-settings-button" onclick="mb.optionsMenu(this); "></span>
		<? endif ?>
		<? if (!empty($arParams['SERVICES'])): ?>
			<span class="webform-small-button webform-small-button-blue bx24-top-toolbar-add" onclick="mb.create(); ">
				...
				new BX.PopupWindowButton({
					...
					events: {
						click: function ()
						{
							...
							BX.ajax({
								method: 'POST',
								url: '<?=$this->__component->getPath() ?>/ajax.php?siteid=<?=urlencode(SITE_ID) ?>&act=password&MAILBOX='+encodeURIComponent(rid),
								data: mb.dialog.getFormData(),
								dataType: 'json',
								onsuccess: function(json)
								{
