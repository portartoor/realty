...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid())
{
	if(isset($_POST['save']) || isset($_POST['apply']))
	{
		...
		foreach ($arEntity as $sEntity)
		{
			...
			{
				if (strpos($k, $sEntity.'_') === 0)
				{
					$_k = substr($k, strlen($sEntity.'_'));
					$arFields[$sEntity][$_k] = $v;
				}
...
/*			else if (!$bConvertCompany && $iCompanyId <= 0)
		...
		*/
		...
		if (empty($arResult['ERROR_MESSAGE']))
		{
			if ($bConvertCompany)
			{
				$iCompanyId = $CCrmCompany->Add($arFields['COMPANY']);
				if($iCompanyId > 0)
				{
			...
			if ($bConvertContact)
			{
				$iContactId = $CCrmContact->Add($arFields['CONTACT']);
				if($iContactId > 0)
				{
			...
			if (!$CrmPerms->HavePerm('DEAL', BX_CRM_PERM_NONE, 'ADD'))
			{
				if ($bConvertDeal)
				{
					...
					$iDealId = $CCrmDeal->Add($arDealFields);
					if($iDealId > 0)
					{
			...
			$CCrmLead->Update($arParams['ELEMENT_ID'], $arFields['LEAD']);
