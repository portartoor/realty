		...
		?><div class="<?=($item_class)?>" id="lenta_item_<?=$arEvent["EVENT"]["ID"]?>"><?
			...
			><?
				...
				elseif (strlen($arEvent["EVENT_FORMATTED"]["MESSAGE"]) > 0)
				{
					...
					{
						?><div id="post_block_check_cont_<?=intval($arEvent["EVENT"]["ID"])?>" class="lenta-info-block-wrapp-full"<?=$strOnClick?>><?
			...
			if (!$arParams["IS_LIST"])
			{
				...
				$db_res->InitFromArray(array_fill(0, intval($arParams["EVENT"]["COMMENTS_COUNT"]), null));
				$db_res->navNum = 1;
