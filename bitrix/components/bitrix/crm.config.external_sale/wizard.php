...
<script>
	...
	$stepTitle = GetMessage("BPWC_WNCW_TITLE_".$currentStep);
	if (empty($stepTitle))
		$stepTitle = GetMessage("BPWC_WNCW_TITLE", array("#STEP#" => $currentStep));
	echo $stepTitle;
	?>');
	top.BX.WindowManager.Get().SetHead('<?= GetMessage("CRM_CES_HEAD_".$currentStep."_".(($id > 0) ? "1" : "0")) ?>');
	...
	if ($currentStep > 1)
	{
		?>
		btns.push({
			'title': "  <  <?= GetMessage("BPWC_WNCW_BACK") ?>  ",
			'id': 'btn-prev',
			'name': 'btn-prev',
	...
	if ($currentStep < 6)
	{
		?>
		btns.push({
			'title': "      <?= GetMessage("BPWC_WNCW_NEXT") ?>    >      ",
			'id': 'btn-next',
			'name': 'btn-next',
			'action': function(){ bxExtSaleWizard.Send("next"); }
		});
		btns.push({
			'title': "<?= GetMessage("BPWC_WNCW_CANCEL") ?>",
			'id': 'btn-cancel',
			'name': 'btn-cancel',
	...
	else
	{
		?>
		btns.push({
			'title': "      <?= GetMessage("BPWC_WNCW_CLOSE") ?>      ",
			'id': 'btn-next',
			'name': 'btn-next',
			'action': function(){ this.parentWindow.Close(); __CrmConfigExtSaleReload(); }
		});
	...
	elseif ($currentStep == 4)
	{
		...
		<script type="text/javascript">
			...
			function ExtSaleDoSync()
			{
				...
				var sendParams = {"id" : <?= $id ?>, "skip_bp" : 'Y', "skip_notify" : 'Y'};
				BX.ajax.get(
					"/bitrix/components/bitrix/crm.config.external_sale/ajax.php",
					sendParams,
					function(v){v = ExtSaleDoPrepareResponse(v); ExtSaleDoSyncResult(v);}
				);
			}
			...
			function ExtSaleDoSyncResult(result)
			{
				...
				if ((result != undefined) && (result["result"] != undefined))
				{
					...
					else
					{
						ExtSaleDoBtns(false);
						document.getElementById("id_es_progress_stop").style.display = "none";
						alert("<?= GetMessage("BPWC_WNCW_SYNC_ERROR") ?>:\n" + result["errors"].replace("<br>", "\n"));
						extSaleSyncStep = 0;
					}
				}
				else
