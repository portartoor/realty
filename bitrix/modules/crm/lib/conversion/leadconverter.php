...
class LeadConverter extends EntityConverter
{
	...
	public function initialize()
	{
		...
		$leadDb = \CCrmLead::GetListEx(array(), array('=ID' => $this->entityID), false, false, array('IS_RETURN_CUSTOMER'));
		$lead = $leadDb->Fetch();
		if(!$lead)
		{
			...
			if(is_array($presentFields))
			{
				...
				if(!empty($fields))
				{
					...
					if($entity->Update($this->entityID, $fields, true, true, array('REGISTER_SONET_EVENT' => true)))
					{
						...
						//Region automation
						if ($statusID !== 'CONVERTED')
							\Bitrix\Crm\Automation\Factory::runOnStatusChanged(\CCrmOwnerType::Lead, $this->entityID);
						//end region
					}
				}
				//region Channel Bindings
				if(isset($this->resultData[\CCrmOwnerType::DealName]))
				{
					DealChannelBinding::copy(
						\CCrmOwnerType::Lead,
						$this->entityID,
						$this->resultData[\CCrmOwnerType::DealName]
					);
				}
				//endregion
			}
