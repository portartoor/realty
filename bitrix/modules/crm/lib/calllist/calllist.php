...
final class CallList
{
	...
	protected function resolveItemFields()
	{
		$itemIds = array();
		foreach ($this->items as $item)
		...
		while($row = $cursor->Fetch())
		{
			...
			$result[$row['ID']]['EDIT_URL'] = \CCrmOwnerType::GetEditUrl($this->getEntityTypeId(), $row['ID']);
		}

		// filling up phones
		$cursor = \CCrmFieldMulti::GetListEx(
			array(),
			array('ENTITY_ID' => \CCrmOwnerType::ResolveName($this->getEntityTypeId()), '@ELEMENT_ID' => $itemIds, '@TYPE_ID' => array('PHONE')),
			false,
			false,
		...
		foreach ($this->items as $item)
		{
			$item->setName($result[$item->getElementId()]['NAME']);
			$item->setCompanyTitle($result[$item->getElementId()]['COMPANY_TITLE']);
			$item->setCompanyPost($result[$item->getElementId()]['POST']);
			$item->setEditUrl($result[$item->getElementId()]['EDIT_URL']);
			if(is_array($result[$item->getElementId()]['PHONES']))
				$item->setPhones($result[$item->getElementId()]['PHONES']);
		}
	}
