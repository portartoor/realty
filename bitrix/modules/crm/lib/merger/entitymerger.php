	...
	protected static function mergeUserFields(array &$seed, array &$targ, array &$fieldInfos, array $options = array())
	{
		...
		foreach($fieldInfos as $fieldID => &$fieldInfo)
		{
			...
			if($typeID === 'file')
			{
				...
				{
					$file = null;
					if(\CCrmFileProxy::TryResolveFile($seed[$fieldID], $file, array('ENABLE_ID' => true, 'ENABLE_UPLOAD' => $enableUpload)))
					{
						$targ[$fieldID] = $file;
					}
				}
				elseif($isMultiple && isset($seed[$fieldID]) && is_array($seed[$fieldID]))
				{
					...
					if(!empty($previousFileIDs))
					{
						foreach($previousFileIDs as $data)
						{
							...
							{
								//HACK: Preserve existed file.
								$targ[$fieldID][] = array('tmp_name' => '', 'old_id' => $data, 'error' => 4);
							}
						}
					}
					...
					{
						$file = null;
						if(\CCrmFileProxy::TryResolveFile($fileID, $file, array('ENABLE_ID' => true, 'ENABLE_UPLOAD' => $enableUpload)))
						{
							$targ[$fieldID][] = $file;
						}
					}
