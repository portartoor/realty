...
class Task extends Activity\Provider\Base
{
	...
	public static function onTaskUpdate($taskId, &$taskFields)
	{
		...
		while($activity = $listIterator->fetch())
		{
			...
			if(isset($activity['BINDINGS']) && count($activity['BINDINGS']) > 0)
			{
				...
				{
					\CCrmActivity::SaveCommunications($activity['ID'], $communications, $activity, true, false);
				}
		...
		if(!$isFound)
		{
			...
			if(isset($activity['BINDINGS']) && count($activity['BINDINGS']) > 0)
			{
				$activityId = \CCrmActivity::add($activity, false, true, array('SKIP_ASSOCIATED_ENTITY' => true, 'REGISTER_SONET_EVENT' => true));
				if($activityId > 0)
				{
					//Stub for communication is required for activity list optimization (see \CCrmActivity::PrepareClientInfos)
					$communications = self::prepareCommunications($activity);
					if(!empty($communications))
					{
						\CCrmActivity::SaveCommunications($activityId, $communications, $activity, true, false);
					}
				}
			}
		}
		...
		$regEvent = true)
	{
		...
		if(isset($activity['BINDINGS']) && count($activity['BINDINGS']) > 0)
		{
			$activityId = \CCrmActivity::Add($activity, $checkPerms, $regEvent, array('SKIP_ASSOCIATED_ENTITY' => true, 'REGISTER_SONET_EVENT' => true));
			if($activityId > 0)
			{
				//Stub for communication is required for activity list optimization (see \CCrmActivity::PrepareClientInfos)
				$communications = self::prepareCommunications($activity);
