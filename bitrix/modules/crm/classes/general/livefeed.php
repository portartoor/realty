	...
	static private function PrepareCounterData($arLogFields)
	{
		...
		if (
			...
			&& in_array(
				...
				array("crm_lead_message", "crm_deal_message", "crm_contact_message", "crm_company_message", "blog_post", "blog_post_important"))
		)
		{
			...
			case "COMMUNICATIONS":
				...
				)
				{
					...
					if (is_array($arCommunication["ENTITY_SETTINGS"]) && isset($arCommunication["ENTITY_SETTINGS"]["COMPANY_TITLE"]))
					{
						...
						{
							$strResult .= '<a class="crm-feed-client-company" href="'.CCrmOwnerType::GetShowUrl(CCrmOwnerType::Company, $arCommunication["ENTITY_SETTINGS"]["COMPANY_ID"]).'">'.$arCommunication["ENTITY_SETTINGS"]["COMPANY_TITLE"].'</a>';
						}
						else
						{
