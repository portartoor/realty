...
class CCrmEMail
{
	...
	public static function imapEmailMessageAdd($msgFields)
	{
		...
		{
			$denyNewLead    = in_array('crm_deny_new_lead', $mailbox['OPTIONS']['flags']);
			...
			if ($isIncome)
			{
				...
				if (CModule::includeModule('mail'))
				{
					...
					$employee = \Bitrix\Mail\MailboxTable::getList(array(
						'select' => array('LOGIN'),
						'filter' => array('!USER_ID' => null, 'ACTIVE' => 'Y', '=LOGIN' => $sender),
						'limit'  => 1,
					))->fetch();
			...
			else
			{
				...
				if (CModule::includeModule('mail'))
				{
					...
					$res = \Bitrix\Mail\MailboxTable::getList(array(
						'select' => array('LOGIN'),
						'filter' => array('!USER_ID' => null, 'ACTIVE' => 'Y', '=LOGIN' => $rcpt),
					));

					while ($employee = $res->fetch())
						$found[] = $employee['LOGIN'];
		...
		$commData   = self::extractCommsFromEmails($commEmails);
		...
		if (empty($userId))
		{
			...
			if (!$denyNewLead)
			{
				...
				$respOption = array_values($respOption);
		...
		if (empty($bindingData) && count($commEmails) > 0)
		{
			...
			if ($isIncome)
			{
				$senderInfo = \CCrmMailHelper::parseEmail($replyTo);
				if (empty($senderInfo['NAME']))
					$senderInfo = \CCrmMailHelper::parseEmail($from);

				$leadName = $senderInfo['NAME'];
			}
			else
			{
				// @TODO: parse body
				$rcptInfo = \CCrmMailHelper::parseEmail($to);

				$leadName = $rcptInfo['NAME'];
			}

			if (empty($leadName))
				$leadName = reset($commEmails);
			...
			$leadFields = array(
				...
				'SOURCE_DESCRIPTION' => $leadDescription,
				'OPENED'             => 'Y',
			...
			foreach ($commEmails as $i => $item)
			{
				$leadFields['FM']['EMAIL'][sprintf('n%u', $i+1)] = array(
					'VALUE_TYPE' => 'WORK',
					'VALUE'      => $item
				);
			}
			...
			if ($leadId > 0)
			{
				...
				$commData = array();
				foreach ($commEmails as $i => $item)
					$commData[] = self::createComm(\CCrmOwnerType::Lead, $leadId, $item);

				$key   = self::prepareEntityKey(\CCrmOwnerType::Lead, $leadId);
				$value = self::createBinding(\CCrmOwnerType::Lead, $leadId);
				$bindingData = array($key => $value);
			}
		}
		...
		{
			$descr     = preg_replace('/\r\n|\n|\r/', '<br/>', htmlspecialcharsbx($body));
			$descrType = \CCrmContentType::Html;
		}
		else
		{
			$descr     = htmlspecialcharsbx($body_bb);
			$descrType = \CCrmContentType::BBCode;
		}
		...
		foreach ($filesData as $fileData)
		{
			...
			if ($fileId > 0)
			{
				...
				$elementId = StorageManager::saveEmailAttachment($fileData, $storageTypeId, $siteId);
				if ($elementId > 0)
					$elementIds[] = (int) $elementId;
			}
		}

		$datetime = !empty($msgFields['FIELD_DATE']) && $DB->isDate($msgFields['FIELD_DATE'], FORMAT_DATETIME)
		...
		if ($activityId > 0)
		{
			if (!empty($commData))
			{
				\CCrmActivity::saveCommunications($activityId, $commData, $activityFields, false, false);
