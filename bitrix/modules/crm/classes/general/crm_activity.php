...
//			}
			...
			if (!isset($fields['STATUS']))
			{
				$fields['STATUS'] = ($fields['COMPLETED'] === 'Y')
					? CCrmActivityStatus::Completed : CCrmActivityStatus::Waiting;
			}
			...
			if(isset($fields['COMPLETED']))
			{
				...
				if(!isset($fields['STATUS']))
				{
					if ($fields['COMPLETED'] === 'Y'
						...
						&& (int)$prevFields['STATUS'] === CCrmActivityStatus::Waiting
					)
	...
	public static function SaveBindings($ID, $arBindings, $registerEvents = true, $checkPerms = true)
	{
		...
		CCrmActivity::DoSaveBindings($ID, array_values($result));
	}
	...
	{
		\Bitrix\Crm\Activity\Provider\Task::onTaskUpdate($taskID, $arTaskFields);
	}
