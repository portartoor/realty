...
$arParams['PATH_TO_DEAL_KANBAN'] = CrmCheckPath('PATH_TO_DEAL_KANBAN', $arParams['PATH_TO_DEAL_KANBAN'], $currentPage);
...
$enableFunnelFilter = !$bInternal && isset($_REQUEST['FL']) && strtoupper($_REQUEST['FL']) === 'Y';
if($enableFunnelFilter)
{
	$dataSourceFilter = null;

	if (is_array($_REQUEST['FILTER']) && !empty($_REQUEST['FILTER']))
	{
		$arFilter = $_REQUEST['FILTER'];
	}
	else
	{
		$enableFunnelFilter = false;
	}
}

$arResult['IS_EXTERNAL_FILTER'] = ($enableWidgetFilter || $enableCounterFilter || $enableFunnelFilter);
...
$arResult['HEADERS'] = array(
	...
	array('id' => 'STAGE_ID', 'name' => GetMessage('CRM_COLUMN_STAGE_ID'), 'sort' => 'stage_sort', 'width' => 200, 'default' => true, 'prevent_default' => false, 'editable' => false, 'type' => 'list')
);
