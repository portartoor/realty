...
class CCrmWebFormFillComponent extends \CBitrixComponent
{
			...
			else
			{
				...
				if($this->arResult['FORM']['IS_PAY'] == 'Y')
				{
					...
					if($resultEntity && $resultEntity->getInvoiceId())
					{
						...
						{
							$httpHost = \Bitrix\Main\Context::getCurrent()->getServer()->getHttpHost();
							$resultRedirectUrl = ($this->request->isHttps() ? 'https' : 'http') . '://' . $httpHost . $resultRedirectUrl;
	...
	{
		$cacheId = $this->getCacheID() . '|' . substr($this->arParams['SECURITY_CODE'], 0, 32) . '|' . $this->getLanguageId();
