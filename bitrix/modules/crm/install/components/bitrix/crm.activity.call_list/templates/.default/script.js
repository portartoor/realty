(function()
{
	...
	CallListActivity.prototype.bindEvents = function()
	{
		...
		if (callButton)
		{
			BX.bind(callButton, 'click', function(e)
			{
				...
				for(dialogId in BX.CrmActivityProvider.dialogs)
				{
					if(BX.CrmActivityProvider.dialogs.hasOwnProperty(dialogId) && BX.CrmActivityProvider.dialogs[dialogId])
					{
						BX.CrmActivityProvider.dialogs[dialogId].close();
					}
				}
				BX.FoldedCallView.getInstance().destroy();
				window.BXIM.startCallList(self.callListId, {
	...
	{
		var gridHandle = BX.Main.gridManager.getById(this.gridId);
