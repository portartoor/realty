...
if(typeof BX.Crm.Component.FormRecurring === 'undefined')
{
	...
	BX.Crm.Component.FormRecurring = function()
	{
		...
		this.construct = function (params)
		{
			...
			this.typeId = params.ENTITY_TYPE_ID || 5;
		...
		this.changeClientData = function(sender, data)
		{
			...
			if (sender.getId() === 'UF_MYCOMPANY_ID')
			{
				...
				if (!data.primaryEntityInfo || mailData[0] == undefined)
				{
					this.existUserMail = false;
				}
				else
				{
					this.existUserMail = true;
				}
				this.toggleMailSender();
			}
			else if (sender.getId() === 'CLIENT')
			{
				if (data.entityInfos instanceof Array && data.primaryEntityTypeName == "COMPANY")
				{
					data.entityInfos.forEach(
						function(entity)
						{
							mailData = entity.getMultiFieldsByType("EMAIL");
							if (mailData[0] !== undefined)
							{
								mailList.push(
									{
										text: mailData[0].VALUE,
										value: mailData[0].ENTITY_ID
									}
								);
							}
						}
		...
		this.bindEvents = function()
		{
			...
			BX.bind(BX('crm-recurring-email').parentNode, 'click', BX.delegate(
				function ()
				{
					if (!this.existUserMail)
					{
						BX('crm-recurring-empty-owner-email').innerHTML = BX.message('CRM_RECURRING_EMPTY_OWNER_EMAIL');
					}
			...
			this.bindInstantChange(BX('daily-interval-day'), moreNull);
			this.bindInstantChange(BX('monthly-month-num-1'), moreNull);
			this.bindInstantChange(BX('monthly-month-num-2'), moreNull);
			this.bindInstantChange(BX('weekly-interval-week'), moreNull);
		...
		this.onExternalEvent = function(params)
		{
			...
			{
				if (value.entityType == this.typeId)
				{
		...
		this.onUpdateHint = function()
		{
			...
			var params = {
				...
				'EVERY_DAY': this.getValueInt('daily-interval-day'),
				'WORKDAY_ONLY': this.getSelectedControlValues('workday-only-select'),
				'DAILY_MONTH_INTERVAL': this.getValueInt('daily-interval-month'),
				'EVERY_WEEK': this.getValueInt('weekly-interval-week'),
				'WEEK_DAYS': this.getWeekDays(), // in this field, values start from 1
				'MONTHLY_DAY_NUM': this.getValueInt('monthly-day-num'),
				'MONTHLY_MONTH_NUM_1': this.getValueInt('monthly-month-num-1'),
				'MONTHLY_WORKDAY_ONLY': this.getSelectedControlValues('monthly-only-select'),
				...
				'YEARLY_DAY_NUM': this.getValueInt('yearly-interval-day'),
				'YEARLY_MONTH_1': this.getValueInt('yearly-month-1'),
				'YEARLY_WEEK_DAY_NUM': this.getValueInt('yearly-week-day-num'),
				'YEARLY_WORKDAY_ONLY': this.getSelectedControlValues('yearly-only-select'),
		...
		this.makeHintText = function (params)
		{
			...
			if(params.PERIOD == this.constants.PERIOD_DAILY)
			{
				...
				var dayNumber = params.EVERY_DAY > 1 ? params.EVERY_DAY +' ' : '';

				if(params.WORKDAY_ONLY == 'Y')
				{
			...
			{
				number = params.EVERY_WEEK > 1 ? params.EVERY_WEEK +' ' : '';
				...
				else
				{
					...
					{
						weekList.push(BX.message('CRM_RECURRING_HINT_WEEKDAY_WD_' + params.WEEK_DAYS[k]));
					}
			...
			else if(params.PERIOD == this.constants.PERIOD_MONTHLY)
			{
				...
				else
				{
					...
					number = BX.message('CRM_RECURRING_HINT_WEEKDAY_NUMBER_' + params.MONTHLY_WEEK_DAY_NUM + weekDayGender);
			...
			else
			{
				...
				{
					number = params.YEARLY_DAY_NUM;
					monthName = BX.message('CRM_RECURRING_HINT_MONTH_' + params.YEARLY_MONTH_1);
				...
				else
				{
					...
					each = BX.message('CRM_RECURRING_HINT_EACH' + weekDayGender);
					monthName = BX.message('CRM_RECURRING_HINT_MONTH_' + params.YEARLY_MONTH_2);
			...
			if (params.END_DATE != '' && till == this.constants.REPEAT_TILL_DATE)
			{
			...
			return BX.message('CRM_RECURRING_HINT_BASE').replace('#ELEMENT#', messageElement).replace('#START#', startText).replace('#END#', constraint);
		};
