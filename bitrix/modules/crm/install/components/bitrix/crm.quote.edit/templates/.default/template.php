...
if(isset($arResult['DEAL_ID']) && $arResult['DEAL_ID'] > 0)
{
	...
	$arFormButtons['custom_html'] = '<input type="hidden" name="quote_id" value="'.$elementID.'"/><input type="hidden" name="deal_id" value="'.$arResult['DEAL_ID'].'"/>';
}
else
{
	$arFormButtons['standard_buttons'] = true;
	$arFormButtons['wizard_buttons'] = false;
	$arFormButtons['custom_html'] = '<input type="hidden" name="quote_id" value="'.$elementID.'"/>';
}
