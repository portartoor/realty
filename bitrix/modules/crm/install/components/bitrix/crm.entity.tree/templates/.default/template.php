							...
							<tr>
								...
								<td>
									<?foreach ($item['FM_VALUES']['PHONE'] as $p => $val):?>
										<a href="javascript:void(0);" class="crm-doc-gray crm-doc-bold crm-doc-clear crm-doc-cart-contact-item-phone" onclick="if(typeof(window['BXIM']) === 'undefined') {<?
										?>window.alert('<?= $lang['CRM_SIP_NO_SUPPORTED']?>'); <?
										?>return;<?
										?>} <?
										?>BXIM.phoneTo('<?=CUtil::JSEscape($val)?>', {<?
										?>ENTITY_TYPE: '<?=CUtil::JSEscape($item['TREE_TYPE'])?>', <?
										?>ENTITY_ID: '<?=CUtil::JSEscape($item['ID'])?>'});"><?= $val?></a>
									<?endforeach;?>
								</td>
							</tr>
						<?endif;?>
					...
					<div class="crm-doc-cart-param">
						<div class="crm-doc-info-progress">
							...
							<table class="crm-doc-info-table">
								...
								<?if ($item['DATE_PAYED'] || $item['DATE_BILL']):?>
								<tr>
									<td><?= $lang['DATE_PAYED']?>:</td>
									<td><?= CrmEntityTreeConvertDateTime($item['PAY_VOUCHER_DATE'] ? $item['PAY_VOUCHER_DATE'] : $item['DATE_BILL'], FORMAT_DATE)?></td>
								</tr>
								<?endif;?>
								<tr>
									<td><?= $lang['SUM']?>:</td>
