...
if($mode === 'SAVE_PDF')
{
	...
	if($quoteID <= 0)
	{
		__CrmQuoteShowEndJsonResonse(array('ERROR'=>'QUOTE_ID IS NOT FOUND!'));
	}
	...
	{
		__CrmQuoteShowEndJsonResonse(array('ERROR'=>'MODULE SALE NOT INCLUDED!'));
	}

	$paySystemID = isset($_POST['PAY_SYSTEM_ID']) ? intval($_POST['PAY_SYSTEM_ID']) : 0;
	if($paySystemID <= 0)
	{
		__CrmQuoteShowEndJsonResonse(array('ERROR'=>'PAY_SYSTEM_ID ID NOT FOUND!'));
	}
	...
	{
		__CrmQuoteShowEndJsonResonse(array('ERROR'=>'QUOTE IS NOT FOUND!'));
	}
	$paymentData = CCrmQuote::PrepareSalePaymentData($quoteFields);
	if(!is_array($paymentData))
	{
		__CrmQuoteShowEndJsonResonse(array('ERROR'=>'COULD NOT PREPARE PAYMENT DATA!'));
	}
	...
	{
		__CrmQuoteShowEndJsonResonse(array('ERROR'=>'COULD NOT FIND PAYMENT SYSTEM ACTION!'));
	}
	...
	{
		__CrmQuoteShowEndJsonResonse(array('ERROR'=>'COULD NOT FIND PAYMENT SYSTEM ACTION FILE!'));
	}
...
if($mode === 'CONVERT')
{
	...
	if($wizard->execute())
	{
		__CrmQuoteShowEndJsonResonse(array('DATA' => array('URL' => $wizard->getRedirectUrl())));
	}
	else
	{
		...
		if($url !== '')
		{
			__CrmQuoteShowEndJsonResonse(array('DATA' => array('URL' => $url)));
		}
		else
		{
