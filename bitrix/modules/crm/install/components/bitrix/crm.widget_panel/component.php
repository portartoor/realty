...
if(Filter::getDateType($arResult['FILTER_FIELDS'], 'PERIOD') === '')
{
	...
	$arResult['FILTER_FIELDS'] = $filterOptions->getFilter();
}
//endregion
	...
	for($j = 0; $j < $cellQty; $j++)
	{
		...
		for($k = 0; $k < $controlQty; $k++)
		{
			...
			if(isset($control['filter']) && is_array($control['filter']) && !empty($control['filter']))
			{
				...
				if($filter->isEmpty())
				{
					Filter::merge($commonFilter, $filter);
				}
				else
				{
					$filter->setContextEntityTypeName($arResult['DEFAULT_ENTITY_TYPE']);
					$filter->setContextEntityID($arResult['DEFAULT_ENTITY_ID']);
				}
			}
			else
			{
