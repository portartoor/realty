	...
	<tr><?
		...
		<td><?
			...
			<? if (CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)) { ?>
				<?=sprintf("Teléfono: %s", CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)); ?><br>
			<? } ?>
...
if (!empty($arBasketItems))
{
			...
			foreach ($arCols as $columnId => $caption)
			{
				...
				switch ($columnId)
				{
					...
					case 'QUANTITY':
						...
						break;
					case 'MEASURE':
						$data = $arBasket["MEASURE_NAME"] ? htmlspecialcharsbx($arBasket["MEASURE_NAME"]) : 'pza';
						break;
					case 'PRICE':
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_LA_TOTAL_SHOW') == 'Y')
	{
		if ($sum < $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = "Sub Total:";
		...
		if (!empty($vats))
		{
			...
			foreach ($vats as $vatRate => $vatSum)
			{
				...
				$arCells[$n][$arColumnKeys[$columnCount-2]] = sprintf("Impuesto de ventas (%s%%):", roundEx($vatRate * 100, SALE_VALUE_PRECISION));
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DISCOUNT_VALUE"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = "Descuento:";
...
<div><?=sprintf('Fecha de pago: %s', ConvertDateTime(CSalePaySystemAction::GetParamValue("DATE_PAY_BEFORE", false), FORMAT_DATE))?></div>
<br>
<? } ?>
