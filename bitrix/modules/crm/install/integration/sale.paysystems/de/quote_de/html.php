...
<html>
<head>
<title>Unverbindliches Angebot</title>
...
<?if (CSalePaySystemAction::GetParamValue('QUOTE_DE_HEADER_SHOW') == 'Y'):?>
<table class="header">
	<tr><?
	...
	<tr>
		<td>
			<span style="font-size: 1.5em; font-weight: bold; text-align: center;">
				<?=sprintf('Unverbindliches Angebot Nr. %s',
...
if (!empty($arBasketItems))
{
	...
	{
		if (CSalePaySystemAction::GetParamValue('QUOTE_DE_COLUMN_'.$column.'_SHOW') == 'Y')
		{
			$arCols[$column] = array(
				'NAME' => CSalePaySystemAction::GetParamValue('QUOTE_DE_COLUMN_'.$column.'_TITLE'),
				'SORT' => CSalePaySystemAction::GetParamValue('QUOTE_DE_COLUMN_'.$column.'_SORT')
			);
		}
	}
			...
			foreach ($arCols as $columnId => $caption)
			{
				...
				switch ($columnId)
				{
					...
					case 'QUANTITY':
						...
						break;
					case 'MEASURE':
						$data = $arBasket["MEASURE_NAME"] ? htmlspecialcharsbx($arBasket["MEASURE_NAME"]) : 'St.';
						break;
					case 'PRICE':
	...
	$items = $n;
	if (CSalePaySystemAction::GetParamValue('QUOTE_DE_TOTAL_SHOW') == 'Y')
	{
			...
			if(!empty($arTaxList))
			{
				foreach($arTaxList as &$arTaxInfo)
				{
					...
					$arCells[$n][$arColumnKeys[$columnCount-2]] = htmlspecialcharsbx(sprintf(
						"%s%s%s:",
						($arTaxInfo["IS_IN_PRICE"] == "Y") ? "inkl." : "zzgl.",
...
<div><?=sprintf('Bitte beachten Sie, dass dieses Angebot bis zum %s gilt.', ConvertDateTime(CSalePaySystemAction::GetParamValue("DATE_PAY_BEFORE", false), FORMAT_DATE))?></div>
<br>
<? } ?>
...
<?if (CSalePaySystemAction::GetParamValue('QUOTE_DE_SIGN_SHOW') == 'Y'):?>
	<? if (!$blank) { ?>
