...
BX.PortalTopMenu = {
	...
	init : function()
	{
		var items = BX('top-menu').querySelectorAll("[data-role='bx-top-menu-item']");
		if (items)
		{
			for(var i=0; i<items.length; i++)
			{
				BX.bind(items[i], "click", function(event){
					BX.PortalTopMenu.itemOnclick(this, event);
				});

				BX.bind(items[i], "touchstart", function(event){
					BX.PortalTopMenu.itemTouchStart(this, event);
				});

				BX.bind(items[i], "mouseover", function(event){
					BX.PortalTopMenu.itemOver(this);
				});

				BX.bind(items[i], "mouseout", function(event){
					BX.PortalTopMenu.itemOut(this, event);
				});
			}
		}
	},
