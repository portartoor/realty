...
if (
	...
	&& $APPLICATION->GetCurPage() != SITE_DIR."desktop.php" 
	&& $APPLICATION->GetCurPage(true) != SITE_DIR."crm/index.php"
)
{
...
elseif ($arResult["PERMISSION"] > "W")
{
	...
	if (
		$USER->IsAdmin() 
		&& COption::GetOptionString("main", "wizard_clear_exec", "N", SITE_ID) != "Y" 
		&& COption::GetOptionString("intranet", "wizard_show_demo_delete", "Y", SITE_ID) != "N" 
		&& $APPLICATION->GetCurPage(true) == SITE_DIR."index.php"
	)
	{
		$gadgetButtons .= '
			<div class="sidebar-block sidebar-help-block">
				...
				<div class="sidebar-block-inner">
					...
					<a class="delete-demo-banner-close" href="javascript:void(0)" onclick="var parNode = this.parentNode.parentNode; BX.ajax.post(
							\''.$this->GetFolder().'/ajax.php\',
							{
								delete_demo_banner: \'Y\',
