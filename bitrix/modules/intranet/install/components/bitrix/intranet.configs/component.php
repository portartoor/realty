...
if (Bitrix\Main\Loader::includeModule("disk"))
{
	...
	$arResult["DISK_LIMIT_PER_FILE"] = array(0 => GetMessage('DISK_VERSION_LIMIT_PER_FILE_UNLIMITED'), 10 => 10, 50 => 50, 100 => 100, 500 => 500);
}

//logo for bitrix24
if ($arResult["IS_BITRIX24"] && $_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid())
{
...
if ($_SERVER["REQUEST_METHOD"] == "POST"  && (isset($_POST["save_settings"]) )&& check_bitrix_sessid())
{
	...
	if ($arResult["IS_BITRIX24"])
	{
		...
		if (isset($_REQUEST["site_title"]) && strlen($_REQUEST["site_title"])>0)
		{
			COption::SetOptionString("bitrix24", "site_title", $_REQUEST["site_title"]);
		}

		if (in_array($arResult["LICENSE_TYPE"], array("team", "company", "edu")))
		{
		...
		if (in_array($arResult["LICENSE_TYPE"], array("team", "company", "demo", "edu", "tf")))
		{
...
if ($arResult["IS_BITRIX24"])
{
	...
	{
		$socnetObj = new \Bitrix\Socialservices\Network();
		$arResult["NETWORK_AVAILABLE"] = $socnetObj->isEnabled() ? "Y" : "N";
	}
