...
?>
	<script type="text/javascript">
		...
		<?if(isset($arParams["ABSENCE_ELEMENT_ID"]) && intval($arParams["ABSENCE_ELEMENT_ID"])>0 || isset($_POST['absence_element_id'])):?>
			myButton.setName('<?echo GetMessage("INTR_ABSENCE_EDIT")?>');
			myPopup.setTitleBar('<?=getMessage('INTR_EDIT_TITLE') ?>');
		<?elseif ($ID > 1):?>
			myButton.setName('<?echo GetMessage("INTR_ABSENCE_ADD_MORE")?>');
			myPopup.setTitleBar('<?=getMessage('INTR_ADD_TITLE') ?>');
		<?else:?>
			myButton.setName('<?echo GetMessage("INTR_ABSENCE_ADD")?>');
			myPopup.setTitleBar('<?=getMessage('INTR_ADD_TITLE') ?>');
		<?endif?>
