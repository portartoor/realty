...
{
	BX.PopupMenu.show("p" + this.id, this.layout.menu, this.menuItems, {
		offsetLeft : 8,
...
GanttProject.prototype.onRowContextMenu = function(event)
{
	...
	BX.PopupMenu.show("p" + this.id, event, this.menuItems, {
   		offsetLeft : 1,
		autoHide : true,
		closeByEsc : true,
   		bindOptions : { forceBindPosition : true },
...
{
	BX.PopupMenu.show(this.id, this.layout.menu, this.menuItems, {
		offsetLeft : 8,
		bindOptions : { forceBindPosition : true },
		events : { onPopupClose: BX.proxy(this.onItemMenuClose, this) },
		chart : this.chart,
...
GanttTask.prototype.onRowContextMenu = function(event)
{
	...
	BX.PopupMenu.show(this.id, event, this.menuItems, {
		offsetLeft : 1,
		autoHide : true,
		closeByEsc : true,
		bindOptions : { forceBindPosition : true },
...
DragDrop.prototype.onDragStop = function(x, y, event)
{
	...
	if (this.error !== null || this.draggableTask.id === this.droppable.id)
	{
		return;
	}
...
DragDrop.prototype.onDragOver = function(destination, x, y)
{
	...
	if (this.draggableTask.projectId !== newProject.id && (!this.draggableTask.canEdit || !newProject.canCreateTasks))
	{
		this.error = true;
	}
