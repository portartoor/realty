		...
		methods: {
			...
			onManagerChange: function(order)
			{
				...
				this.getManager().each(function(item){
					...
					item.sort(i++); // update sort for each item
				});
	...
	BX.Tasks.Component.TasksWidgetCheckList.ItemManager = BX.Tasks.Util.ItemSet.extend({
		...
		methods: {
			...
			toggleItemMode: function(apply, item)
			{
				...
				{
					item.control('new-title').value = item.data().TITLE; // rollback
					item.toggleMode();
				}
			},
		...
		methods: {
			...
			sort: function(number)
			{
				if(typeof number != 'undefined')
				{
					...
					this.data().SORT = number;
					this.control('number').innerHTML = number + 1;
					this.control('sort-fld').value = number;
