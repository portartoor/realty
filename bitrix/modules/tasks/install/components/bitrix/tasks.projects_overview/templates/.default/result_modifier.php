...
{
	$image = \Bitrix\Tasks\UI::getAvatarFile($arProject['IMAGE_ID'], array('WIDTH' => 50, 'HEIGHT' => 50));
	$arImage = array(
		'FILE' => $image['ORIGIN'],
		'IMG' => '<a href="'.$arProject['PATHES']['IN_WORK'].'" class="profile-menu-avatar" '.($image['RESIZED']['SRC'] ? 'style="background:url(\''.htmlspecialchars($image['RESIZED']['SRC']).'\') no-repeat center center; "' : '').'></a>',
	);
