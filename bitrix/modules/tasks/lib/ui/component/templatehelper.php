...
final class TemplateHelper
{
	...
	public function fillTemplate($template, $data)
	{
		...
		foreach($data as $k => $v)
		{
		    ...
		    {
			    $v = \Bitrix\Tasks\UI::sanitizeString($v, array('a' => array('href'), 'img' => array('src')));
			    $v = \Bitrix\Tasks\UI::convertBBCodeToHtml($v, array('PRESET' => 'BASIC'));
		    }

			$replacement['{{{'.$k.'}}}'] = $v;
			$replacement['{{'.$k.'}}'] = htmlspecialcharsbx(strip_tags($v));
		}
