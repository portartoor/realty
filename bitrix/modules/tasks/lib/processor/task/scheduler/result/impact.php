...
final class Impact extends \Bitrix\Tasks\Processor\Task\Result\Impact
{
	...
	public function getDataPristine()
	{
		...
		{
			if((string)$this->data['START_DATE_PLAN'] == '')
			{
				/** @var DateTime $createdDate */
				$createdDate = $this->data['CREATED_DATE'];
				if((string) $createdDate != '')
				{
					if(!($createdDate instanceof DateTime))
					{
						$createdDate = DateTime::createFromUserTimeGmt($createdDate);
					}
				}
				else
				{
					$createdDate = Calendar::getInstance()->getStartOfCurrentDayGmt();
				}

				$this->setStartDatePlanUserTimeGmt($createdDate->toStringGmt(), false);
			}
			if((string)$this->data['END_DATE_PLAN'] == '')
			{
				// if there is no end date, we could try to set end date as end of the current day
				$this->setEndDatePlanUserTimeGmt(Calendar::getInstance()->getEndOfCurrentDayGmt()->toStringGmt(), false);
			}
	...
	public function setDataUpdated(array $data)
	{
		...
		if(array_key_exists('START_DATE_PLAN', $data) || array_key_exists('END_DATE_PLAN', $data) || array_key_exists('MATCH_WORK_TIME', $data))
		{
			...
			if($startDate || $endDate) // smth were changed, do job
			{
				...
				{
					$this->setStartDatePlanUserTimeGmt($startDate->toStringGmt());
					$this->setEndDatePlanUserTimeGmt($endDate->toStringGmt());
				...
				elseif($startDate) // start changed => resize to left
				{
					...
					$this->matchWorkingTime($startDate);
				}
				elseif($endDate) // end changed => resize to right
				{
					$this->setEndDatePlanUserTimeGmt($endDate->toStringGmt());

					$this->matchWorkingTime(null, $endDate);
				}
			}
		}
	}
	...
	public function getUpdatedData()
	{
		...
		return array(
			'START_DATE_PLAN' => $start->toString(),
			'END_DATE_PLAN' => $end->toString(),
			'DURATION_PLAN_SECONDS' => $this->calculateDuration(),

			'START_DATE_PLAN_STRUCT' => $start->getTimeStruct(),
			'END_DATE_PLAN_STRUCT' => $end->getTimeStruct(),
		);
	}
	...
	private function matchWorkingTime(DateTime $startDate = null, Datetime $endDate = null, $duration = null)
	{
		...
		else
		{
			...
			{
				//_print_r('New: '.$startDate.' - '.$endDate.' '.$duration);

				$this->correctWorkTime($startDate, $endDate, $duration);

				//_print_r('New: '.$this->getStartDatePlan().' - '.$this->getEndDatePlan().' '.$this->calculateDuration());
			}
			elseif($startDate)
			{
				$this->setStartDatePlanUserTimeGmt($calendar->getClosestWorkTime($startDate, true)->toStringGmt());
