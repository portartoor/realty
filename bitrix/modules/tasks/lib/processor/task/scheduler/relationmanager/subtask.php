...
final class SubTask extends \Bitrix\Tasks\Processor\Task\Scheduler\RelationManager
{
	 ...
	 */
	private function updateDateBoundary($parentId, $id, $itemData, $miniMaxes, $isMin)
	{
	...
	private function shiftSubTree(Impact $rootImpact, Fragment $globalTree)
	{
		...
		if($deltaStart == INF || $deltaEnd == INF)
		{
	 ...
	 * @return array
	 */
	private function getInitialBoundaries($id, $fragment)
	{
		$min = null;
		$max = null;
	...
	private function resizeBrackets(Fragment $fragment)
	{
		...
		$miniMaxes = array();
		$walkResult = $fragment->walkDepth(function($item, $id, $itemData, $parentId) use (&$miniMaxes, $scheduler, $fragment, $debug) {
			...
			if(!isset($miniMaxes[$parentId]))
			{
				...
				$miniMaxes[$parentId] = $this->getInitialBoundaries($parentId, $fragment);
			...
			$newBoundary = $this->updateDateBoundary(
				$parentId,
				$id,
				$itemData,
				$miniMaxes,
		...
		foreach($miniMaxes as $taskId => $dates)
		{
			...
			if($taskData['INHERIT_DATES'] == 'Y') // resize only "bracket" tasks
			{
				...
				{
					if($this->isTaskFinite($taskData))
					{
						// this task is being moved for the first time
						if($taskData['START_DATE_PLAN']->isNotEqualTo($dates['MIN']) || $taskData['END_DATE_PLAN']->isNotEqualTo($dates['MAX']))
						{
							$taskData['ID'] = $taskId;
