...
class Chat
{
	...
	public function markSpamAndFinish($userId)
	{
		...
		{
			list(,, $chatId) = explode('|', $session->getData('USER_CODE'));
	...
	public function startSessionByMessage($userId, $messageId)
	{
		...
		$message['PARAMS']['CLASS'] = 'bx-messenger-content-item-system';
	...
	public function setPauseFlag($params)
	{
		...
		else
		{
			\CIMChat::AddMessage(Array(
				"TO_CHAT_ID" => $this->chat['ID'],
				"MESSAGE" => Loc::getMessage('IMOL_CHAT_PIN_OFF'),
				"SYSTEM" => 'Y',
			));
		}
	...
	public static function sendRatingNotify($type, $sessionId, $rating, $toUserId, $fromUserId = null)
	{
		...
		$userViewChat = \CIMContactList::InRecent($toUserId, IM_MESSAGE_CHAT, $sessionId);
		\CIMNotify::Add(array(
			"TO_USER_ID" => $toUserId,
