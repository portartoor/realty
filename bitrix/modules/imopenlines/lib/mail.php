...
class Mail
{
	...
	public static function sendOperatorAnswer($sessionId)
	{
		...
		$title = Loc::getMessage('IMOL_MAIL_HISTORY_TITLE', Array('#SITE_URL#' => $widgetUrlParsed['host'], '#SESSION_ID#' => $sessionId));
		$actionTitle = Loc::getMessage('IMOL_MAIL_ANSWER_ACTION_TITLE');
		$actionDesc = Loc::getMessage('IMOL_MAIL_HISTORY_ACTION_DESC', Array('#SESSION_ID#' => $sessionId));
		...
		$event = new \CEvent;
		$event->Send("IMOL_OPERATOR_ANSWER", SITE_ID, $arFields, "N");
	...
	public static function prepareOperatorAnswerForTemplate($sessionId, $setSendFlag = true)
	{
		...
		list(,,$chatId) = explode('|', $session['USER_CODE']);
		...
		$messages = self::prepareMessagesForTemplate($session, $result);
	...
	public static function sendSessionHistory($sessionId, $email)
	{
		...
		$title = Loc::getMessage('IMOL_MAIL_HISTORY_TITLE', Array('#SITE_URL#' => $widgetUrlParsed['host'], '#SESSION_ID#' => $sessionId));
		$actionTitle = Loc::getMessage('IMOL_MAIL_HISTORY_ACTION_TITLE');
		$actionDesc = Loc::getMessage('IMOL_MAIL_HISTORY_ACTION_DESC', Array('#SESSION_ID#' => $sessionId));
		...
		$event = new \CEvent;
		$event->Send("IMOL_HISTORY_LOG", SITE_ID, $arFields, "N");
	...
	private static function prepareMessagesForTemplate($session, $history)
	{
		...
		foreach ($history['message'] as $messageId => $message)
		{
			...
			{
				$authorName = $isYou? Loc::getMessage('IMOL_MAIL_AUTHOR_YOU'): \Bitrix\Im\User::getInstance($message['senderId'])->getFullName();
			...
			{
				$messageDate = \FormatDate(Loc::getMessage('IMOL_MAIL_TIME_FORMAT'), $message['date']+intval($userTzOffset));
			}
			else
			{
				$messageDate = \FormatDate(Loc::getMessage('IMOL_MAIL_DATETIME_FORMAT'), $message['date']+intval($userTzOffset));
			}
			...
			else
			{
				...
				{
					$messageText .= ' ['.Loc::getMessage('IMOL_MAIL_FILE').']';
