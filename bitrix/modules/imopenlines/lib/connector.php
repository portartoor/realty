...
class Connector
{
	...
	public function addMessage($params)
	{
		...
		if (strlen($addMessage["MESSAGE"]) <= 0 && empty($addMessage["PARAMS"]['FILE_ID']) && empty($addMessage["ATTACH"]))
		{
			if ($session->isNowCreated())
			{
				$session->finish(true, true);
			}
			return false;
		}
		...
		if ($session->getData('WAIT_VOTE') == 'Y')
		{
			$voteValue = 0;
	...
	public static function onMessageUpdate($messageId, $messageFields, $flags)
	{
		...
		)
		{
			if ($messageFields['SYSTEM'] != 'Y' && self::isEnableSendMessageWithSignature($connectorId) && $messageFields['AUTHOR_ID'] > 0)
			{
				...
				$flags['TEXT'] = '[b]'.$user->getFullName(false).':[/b]'.(strlen($flags['TEXT'])>0? '[br] '.$flags['TEXT']: '');
			}
			...
			{
				$connector = new \Bitrix\ImConnector\Output($connectorId, $lineId);
	...
	public static function onMessageSend($messageId, $messageFields)
	{
		...
		else
		{
			...
			if ($messageFields['SYSTEM'] != 'Y' && self::isEnableSendMessageWithSignature($connectorId) && $messageFields['AUTHOR_ID'] > 0)
			{
				$messageFields['MESSAGE'] = '[b]'.$user->getFullName(false).':[/b]'.(strlen($messageFields['MESSAGE'])>0? '[br] '.$messageFields['MESSAGE']: '');
			}
