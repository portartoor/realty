	...
	public function saveForm($type, $fields)
	{
		...
		else if ($type == self::FORM_HISTORY)
		{
			...
			if (isset($fields['EMAIL']) && $chatFieldSession['SESSION_ID'])
			{
				...
				$messageParams = Array(
					"FROM_USER_ID" => $this->userId,
					"MESSAGE" => '[B]'.Loc::getMessage('IMOL_LCC_FORM_HISTORY', Array("#LINK#" => "[URL=/online/?IM_HISTORY=imol|".$chatFieldSession['SESSION_ID']."]".$chatFieldSession['SESSION_ID']."[/URL]")).'[/B]',
					"ATTACH" => $attach,
					"SKIP_CONNECTOR" => 'Y',
					"SYSTEM" => 'Y',
				);
		...
		// update crm entity
		if (empty($userUpdate) && isset($fields['EMAIL']) && $fields['EMAIL'] != $user->getEmail())
		{
			if (!$session)
			{
				$session = new Session();
