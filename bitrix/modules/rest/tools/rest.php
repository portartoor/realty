	...
	switch($action)
	{
					...
					if($appDetailInfo)
					{
						...
						elseif($installResult['result'])
						{
							...
							if($addResult->isSuccess())
							{
								...
								//\Bitrix\Rest\Marketplace\Client::setInstall($appFields['CODE'], 'Y');

								$result = array('success' => 1);
								if($menu)
								{
									$result['redirect'] = '/marketplace/app/'.$appId.'/';
								}
							}
							else
							{
			...
			if($admin)
			{
				...
				if($appInfo && $appInfo['STATUS'] === \Bitrix\Rest\AppTable::STATUS_LOCAL)
				{
					...
					elseif(!empty($appInfo['URL_INSTALL']))
					{
						...
						$result = array(
							'success' => 1,
							'redirect' => '/marketplace/app/'.intval($appInfo['ID']).'/'
						);
					}
				}
				else
		...
		case "set_app_rights":
			if($admin)
			{
				...
				if($appId > 0)
				{
					\Bitrix\Rest\AppTable::setAccess($app_id, $_POST["rights"]);
					$result = array('success' => 1);
				}
			}
			else
