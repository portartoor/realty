...
if($arParams['ID'] <= 0)
{
	...
	$arParams['ID'] = intval($arVariables['id']);
}

$request = \Bitrix\Main\Context::getCurrent()->getRequest();

$arParams['POPUP'] = isset($arParams['POPUP']) ? $arParams['POPUP'] : ($request->isPost() && $request['popup'] == 1 && check_bitrix_sessid());
...
if($arParams['PLACEMENT'] === \Bitrix\Rest\PlacementTable::PLACEMENT_DEFAULT && empty($arParams['PLACEMENT_OPTIONS']))
{
	...
	$arParams['PLACEMENT_OPTIONS'] = array_diff_key($requestOptions, array_flip(\Bitrix\Main\HttpRequest::getSystemParameters()));
}
...
)
{
	...
	if ($bHasAccess && strlen($arResult['APP_NAME']) > 0)
	{
	...
	// this is a first run for the application and the current user
		...
		)
		{
			...
			\Bitrix\Rest\AppTable::update($arApp['ID'], array(
				'STATUS' => $arApp['STATUS'],
	...
	// additional actions
		if($arResult['APP_STATUS']['PAYMENT_ALLOW'] == 'Y' && isset($_REQUEST['action']) && check_bitrix_sessid())
		{
			if($_REQUEST['id'] != $arParams['ID'])
			{
				return;
			}
	...
	// get and parse application URL
		...
		{
			$APPLICATION->RestartBuffer();
			$arResult['POPUP_TITLE'] = $arResult['APP_NAME'];
		}
		else
		{
