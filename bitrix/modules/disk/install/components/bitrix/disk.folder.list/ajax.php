...
class DiskFolderListAjaxController extends \Bitrix\Disk\Internals\Controller
{
			...
			foreach($objectNewRights as $accessCode => $right)
			{
				...
				if($alreadySpecific)
				{
					$newRights[] = array(
						'ACCESS_CODE' => $accessCode,
						'TASK_ID' => $right['right']['id'],
						'DOMAIN' => !empty($specificRights[$accessCode . '-' . $right['right']['id']]['DOMAIN'])?
							$specificRights[$accessCode . '-' . $right['right']['id']]['DOMAIN'] : null,
					);
				}
	...
	private function getChangedDomainRights(array $specificRights, array $newRights)
	{
	...
	protected function processActionGetDetailSettingsExternalLinkForceCreate()
	{
		$this->processActionGetDetailSettingsExternalLink(true);
	}

	protected function processActionGetDetailSettingsExternalLink($forceCreate = false)
	{
		/** @var File $file */
		/** @var ExternalLink $extLink */
		list($file, $extLink) = $this->getFileAndExternalLink();
	...
	protected function processActionCalculateFileSizeAndCount()
	{
		$this->checkRequiredPostParams(array('folderId'));
		if($this->errorCollection->hasErrors())
		{
			$this->sendJsonErrorResponse();
		}
		/** @var Folder $folder */
		$folder = Folder::loadById((int)$this->request->getPost('folderId'), array('STORAGE'));
		if(!$folder)
		{
