...
<table cellspacing="0" class="bx-disk-interface-grid<?if($arResult["OPTIONS"]["theme"] <> '') echo " bx-disk-interface-grid-theme-".$arResult["OPTIONS"]["theme"]?>" id="<?=$arParams["GRID_ID"]?>">
	<thead>
	<tr class="bx-disk-grid-gutter" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.settingsMenu">
...
<?endforeach?>
	...
	<tr class="bx-disk bx-disk-grid-head" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.settingsMenu"<?if($USER->IsAuthorized()):?> ondblclick="bxGrid_<?=$arParams["GRID_ID"]?>.EditCurrentView()"<?endif?>>
