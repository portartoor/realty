	...
	private static function getDataToCheck($messageId)
	{
		...
		if($messageId > 0 && Loader::includeModule("forum"))
		{
			if(!array_key_exists($messageId, self::$messages))
			{
				...
				{
					$dbRes = \CForumMessage::getListEx(array("ID" => "ASC"), array(
						"TOPIC" => "Y",
					...
					{
						$messages["M" . $res["ID"]] = array_intersect_key($res, array(
							"ID" => "",
							"TOPIC_ID" => "",
							"FORUM_ID" => "",
							"USER_ID" => "",
						...
						{
							$topics["T" . $res["TOPIC_ID"]] = array(
								"TITLE" => $res["TITLE"],
								"USER_ID" => $res["USER_START_ID"],
								"XML_ID" => $res["TOPIC_XML_ID"],
								"SOCNET_GROUP_ID" => $res["TOPIC_SOCNET_GROUP_ID"],
								"OWNER_ID" => $res["TOPIC_OWNER_ID"]
							);
						}
					}
					if(!empty($messages))
