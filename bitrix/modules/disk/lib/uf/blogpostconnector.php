...
use Bitrix\Socialnetwork\CommentAux\FileVersion;
...
final class BlogPostConnector extends Connector
{
	...
	public function addComment($authorId, array $data)
	{
		...
		$commentFields = Array(
			...
			"POST_TEXT" => FileVersion::getPostText(),
		...
		$provider = \Bitrix\Socialnetwork\CommentAux\Base::init(\Bitrix\Socialnetwork\CommentAux\FileVersion::getType(), array(
			'liveParamList' => array(
				'userId' => $authorId,
		...
		$query = \CSocNetLog::getList(
			array("ID" => "DESC"),
			array("EVENT_ID" => array("blog_post", "blog_post_important"), "SOURCE_ID" => $this->entityId),
			false,
			false,
			array("ID", "TMP_ID")
		);
