...
class UrlManager implements IErrorable
{
	 ...
	 * @return string
	 */
	public static function getUrlToShowFileByService($fileId, $service)
	{
		return static::getUrlDocumentController('', array(
	 ...
	 * @return string
	 */
	public static function getUrlFocusController($action, array $params = array())
	{
		$params['action'] = $action;
		$params['ncc'] = 1;
		return '/bitrix/tools/disk/focus.php?' . http_build_query($params);
	}
	...
	private function resolvePath(Storage $storage, $path, $lookUpFromFolderId, $lastPart = FolderTable::TYPE_FOLDER)
	{
		...
		if($path === 'index.php' || !is_string($path) || $path === '' )
		{
