...
class SmartMigrationWebdav
{
	...
	protected function setSymbolicLinks()
	{
		...
		if($this->isMysql)
		{
			$sql = "
			UPDATE b_disk_object obj_link,
			(SELECT ID FROM b_disk_object l WHERE l.REAL_OBJECT_ID IS NOT NULL AND NOT EXISTS(SELECT 'x' FROM b_disk_object o WHERE o.ID = l.REAL_OBJECT_ID)) bad_link
				SET obj_link.REAL_OBJECT_ID = obj_link.ID
				WHERE obj_link.ID = bad_link.ID
			";
		}
		elseif($this->isOracle || $this->isMssql)
		{
