...
class CVoxImplantMain
{
	...
	public static function UpdateChatInfo($dialogId, array $additionalData)
	{
		...
		if (substr($dialogId, 0, 4) == 'chat')
		{
			...
			$updateResult = IM\Model\ChatTable::update(
				$chatId,
				array(
	...
	public static function CallStart($callId, $userId, $callDevice = 'WEBRTC', $external = false)
	{
		...
		CVoxImplantCrmHelper::UpdateCallResponsible($callId, $userId);
	...
	{
		return CVoxImplantHttp::GetPortalType() == CVoxImplantHttp::TYPE_BITRIX24? '/telephony/': '/services/telephony/';
	}
