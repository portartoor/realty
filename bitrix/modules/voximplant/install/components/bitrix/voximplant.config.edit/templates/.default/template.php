...
$resultIncludeBitrix24 = CModule::IncludeModule('bitrix24');
...
if ($resultIncludeBitrix24 && $arResult["TRIAL_TEXT"])
{
		...
		<div class="tel-set-cont-block">
					...
					<div class="tel-set-item-cont-block">
						...
						<? if(!\Bitrix\Voximplant\Ivr\Ivr::isEnabled()): ?>
							<div class="tel-lock-holder-select" title="<?=GetMessage("VI_CONFIG_LOCK_ALT")?>">
								<div onclick="viOpenTrialPopup('vi_crm_source')" class="tel-lock <?=(CVoxImplantAccount::IsDemo()? 'tel-lock-demo': '')?>"></div>
							</div>
							<script>
								BX.bind(BX('vi-set-ivr'), 'change', function(e)
								{
									...
									viOpenTrialPopup('vi_crm_source');
								});
							</script>
						<? endif ?>
					</div>
					...
					<div class="tel-set-item-cont">
						...
						<script type="text/javascript">
							BX.bind(BX('vi_crm_create'), 'change', function(e){
								...
								{
									BX('vi_crm_source').style.height = '55px';
								}
								else
								{
						...
						<div id="vi_crm_source"  class="tel-set-item-select-block tel-set-item-crm-rule" style="background-color: #fff; <?=($arResult["ITEM"]["CRM_CREATE"] != CVoxImplantConfig::CRM_CREATE_NONE? 'height: 55px;': '')?>">
							...
							<select class="tel-set-inp tel-set-item-select" name="CRM_SOURCE" id="vi_crm_source_select">
								<?foreach ($arResult['CRM_SOURCES'] as $ii => $k):?>
									<option value="<?=$ii?>"<?=($ii == $arResult["ITEM"]["CRM_SOURCE"] ? " selected" : "")?>><?=$k?></option>
								<?endforeach;?>
							</select>
							<?if (!CVoxImplantAccount::IsPro() || CVoxImplantAccount::IsDemo()):?>
							<div class="tel-lock-holder-select" title="<?=GetMessage("VI_CONFIG_LOCK_ALT")?>"><div onclick="viOpenTrialPopup('vi_crm_source')" class="tel-lock <?=(CVoxImplantAccount::IsDemo()? 'tel-lock-demo': '')?>"></div></div>
							<?endif;?>
						</div>
			...
			<div class="tel-set-item">
				...
				<div class="tel-set-item-cont-block">
					<div class="tel-set-cont-item-title"><?=GetMessage("VI_CONFIG_ROUTE_TO_SELECT")?>
						<select id="vi-group-id-select" class="tel-set-inp tel-set-item-select" name="QUEUE_ID">
						...
						<span id="vi-group-show-config" class="tel-set-group-show-config"><?=GetMessage('VI_CONFIG_GROUP_SETTINGS')?></span>
					</div>
					<div class="tel-set-item-select-block">
					...
					<?endif;?>
					<div id="vi-group-settings-placeholder" class="tel-set-group-settings-container" style="max-height: 0;">
					</div>
				</div>
			</div>
			<div class="tel-set-item">
				...
				<div class="tel-set-item-cont-block">
					...
					<div class="tel-set-item-select-block" style="margin-bottom: 0;">
						...
						<label class="tel-set-cont-item-title" for="vi_recording_notice" style="display: inline-block; font-weight: normal;"><div class="tel-set-item-select-text"><?=GetMessage("VI_CONFIG_EDIT_RECORD_NOTICE")?></div></label>
					</div>
					<div class="tel-set-item-cont">
			...
			<div class="tel-set-item">
				...
				<div class="tel-set-item-cont-block">
					...
					<?if (!CVoxImplantAccount::IsPro() || CVoxImplantAccount::IsDemo()):?>
					<div class="tel-lock-holder-title" title="<?=GetMessage("VI_CONFIG_LOCK_ALT")?>"><div onclick="viOpenTrialPopup('vi_vote')"  class="tel-lock <?=(CVoxImplantAccount::IsDemo()? 'tel-lock-demo': '')?>"></div></div>
					<?endif;?>
				...
				<div class="tel-set-item-cont-block">
					...
					<div class="tel-set-item-cont">
						...
						<div class="tel-set-melody-block">
							...
							<span class="tel-set-file-wrap"><?$APPLICATION->IncludeComponent('bitrix:main.file.input', '.default',
								array(
...
<!-- //melody -->
		...
		<div class="tel-set-footer-btn">
			<span class="webform-button webform-button-accept" onclick="BX.submit(BX('config_edit_form'))">
...
<script>
	...
	BX.ready(function(e)
	{
		BX.VoxImplantConfigEdit.bindEvents();
	});

	<?if($arResult["SIP_CONFIG"]['TYPE'] == CVoxImplantSip::TYPE_CLOUD):?>
		BX.VoxImplant.config.sip.initHandlers();
	<?endif?>
	<?if($arResult['ITEM']['PORTAL_MODE'] == CVoxImplantConfig::MODE_LINK):?>
	BX.ready(function(){
		BX.ViCallerId.init({
