	...
	function InstallDB($params = Array())
	{
		...
		if (!IsModuleInstalled('bitrix24'))
		{
			...
			foreach ($dirList as $dirElement)
			{
				...
				if (class_exists('\\Bitrix\\ImBot\\Bot\\'.$className) && method_exists('\\Bitrix\\ImBot\\Bot\\'.$className, 'getAutoInstallStatus'))
				{
					if (call_user_func_array(array('\\Bitrix\\ImBot\\Bot\\'.$className, 'getAutoInstallStatus'), Array()))
					{
						CAgent::AddAgent('\\Bitrix\\ImBot\\Bot\\'.$className.'::register(Array("AGENT" => "Y"));', "imbot", "N", 3600, "", "Y", ConvertTimeStamp(time()+CTimeZone::GetOffset()+3600, "FULL"));
					}
				}
			}
			if (IsModuleInstalled('intranet'))
			{
				...
				if ($languageId == 'ru')
				{
					CAgent::AddAgent('\\Bitrix\\ImBot\\Bot\\Properties::register(Array("AGENT" => "Y"));', "imbot", "N", 3600, "", "Y", ConvertTimeStamp(time()+CTimeZone::GetOffset()+3600, "FULL"));
				}
				else if ($languageId == 'ua')
				{
					CAgent::AddAgent('\\Bitrix\\ImBot\\Bot\\PropertiesUa::register(Array("AGENT" => "Y"));', "imbot", "N", 3600, "", "Y", ConvertTimeStamp(time()+CTimeZone::GetOffset()+3600, "FULL"));
				}
				if (in_array($languageId, Array('ru', 'kz', 'ua')))
				{
					CAgent::AddAgent('\\Bitrix\\ImBot\\Bot\\Marta::register(Array("AGENT" => "Y"));', "imbot", "N", 3600, "", "Y", ConvertTimeStamp(time()+CTimeZone::GetOffset()+3600, "FULL"));
				}
			}
		}
