...
<form name="<?=$arResult["FORM_ID"]?>" id="<?=$arResult["FORM_ID"]?>" action="<?=$url?>" method="POST">
	...
	<?if ($can["EDIT"]) : ?><input type="hidden" name="data[SE_AUDITOR][]" value="" /><input type="hidden" name="data[PRIORITY]" value="<?=($task["PRIORITY"] == CTasks::PRIORITY_LOW ? CTasks::PRIORITY_LOW : CTasks::PRIORITY_AVERAGE)?>" /><? endif; ?>
