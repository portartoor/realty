<?
	...
	class SynApp extends SyncRequestsInClass {
		...
		private $StopClassFile = "/syn_request_in_stop"; 
		
		function __construct(){
			parent::__construct();
			$this->MyTime = time();
			$this->Path = $_SERVER["DOCUMENT_ROOT"]."upload/intrum/".$data_res["file"].$this->Path;
			if(!file_exists($this->Path)){die("File not found: ".$this->Path);}
			
			$this->StopFile = $_SERVER["DOCUMENT_ROOT"]."upload/cron".$this->StopFile.".txt";
			...
			$this->StopClassFile = $_SERVER["DOCUMENT_ROOT"]."upload/cron".$this->StopClassFile.".txt";
			
			if(file_exists($this->StopClassFile)){
				if((time()-filemtime($this->StopClassFile))>/*3600*/0)
				{
