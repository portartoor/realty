<? 
	...
	class Syn1c {
		...
		private function SendRequestsClose(){
			...
			$Query = HlBlockElement::GetList(
				...
				array(),
				array(
					">=UF_DATE_CLOSE".$postfix=> date("d.m.Y", strtotime("-1 week"))." 00:00:00", 
					"<=UF_DATE_CLOSE".$postfix=> date("d.m.Y")." 23:59:59"
				),
				array(),
				10
			);
		...
		private function SendRequests(){
			...
			{	
				...
				$Query = HlBlockElement::GetList(
					...
					array(
						/*array(
							"LOGIC" => "OR",
							array("UF_ID".$postfix => false),
							array("=UF_ID".$postfix => 0)
						),*//*"ID"=>8,*/
			...
			while($Answer = $Query->Fetch()){
				...
				foreach($Answer as $key => $value){
					...
					{
						if($key=="UF_LAND_OWNER_TYPE")
							$key_req="UF_LAND_O_TYPE";
						if($key=="UF_NARUZHN_REKLAMA")
							$key_req="UF_N_REKLAMA";
					}
					if($this->UserTypeEntity[$key_req] == "boolean") {
						$value = ($value == 1 ? "Да" : "Нет");
						if(in_array($key,Array("UF_ENTRANCE","UF_CONSTRUCT_PERM"))&&$value=="Нет") 
							$value="";
					} else if($this->UserTypeEntity[$key_req] == "enumeration"&&$value!=""&&$value!=0){
						$QueryFieldEnum = CUserFieldEnum::GetList(
							array(),
							array(
								"USER_FIELD_NAME" => $key_req.$postfix,
								"ID" => $value
							)
						)->Fetch();
					...
					if($this->UserTypeEntity[$key_req] == "enumeration"&&strval($value)=="0")
					{
						$value="";	
						//echo $key." !".$value."!<br>";
					}
					if($key == "UF_REALTY_TYPE"){
						...
						foreach($value as $keyFile => $valueFile){
							...
							if($key=="UF_PHOTO_PREVIEW")
							{
								...
								$desc_arr["DOC"]=0;
								$desc_arr["PLAN"]=0;
								if(!isset($desc_arr["USER"])&&!isset($desc_arr["user"]))
									$desc_arr["USER"]=$Answer["UF_AGENT"];
							}
							...
							if(!isset($desc_arr["SEND"]))
									$desc_arr["SEND"]=1;
							if(!isset($desc_arr["CHANGED"]))
									$desc_arr["CHANGED"]=1;
					...
					if(in_array($key,Array("UF_ID"))&&$value==0){
						$value = "";
					}
				...
				if($Project->s_name=="domofey")
				{
					$Item->addChild("UF_ENUMERATE_PHOTO","");
				}
				$SoapAnswer = null;
				try {
					if($this->RequestId>0)
	...
	{
		LocalRedirect("syn_1c.php?args[]=".$_GET["args"][0]."&project=domofey");
	}
?>
