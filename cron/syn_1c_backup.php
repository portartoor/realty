<?
	...
	class Syn1c {
		...
		function __construct($Args){
			...
			// debug
			//
			$this->DebugFilePath = __DIR__."/syn1c/";
		...
		private function DbgFile($Type = "undenfined",$Id = "",$Data = ""){
			if($this->Debug){
				file_put_contents($this->DebugFilePath.$Type."_request_".$Id.".xml",$Data);
			}
		}
		...
		private function SendRequests(){
			...
			while($Answer = $Query->Fetch()){
				...
				foreach($Answer as $key => $value){
					...
					} else if(in_array($key,array("UF_PHOTOS","UF_DOCS","UF_PLAN_PHOTOS"))){
						$ItemFile = $Item->addChild($key);
						foreach($value as $keyFile => $valueFile){
							$ItemFile->addChild("item","http://portal.invent-realty.ru".CFile::GetPath($valueFile));
						}
						continue;
